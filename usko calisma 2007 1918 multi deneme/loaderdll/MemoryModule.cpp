	...
	if (directory->Size > 0) {
		...
		for (; !IsBadReadPtr(importDesc, sizeof(IMAGE_IMPORT_DESCRIPTOR)) && importDesc->Name; importDesc++) {
			...
			for (; *thunkRef; thunkRef++, funcRef++) {
				...
				if (*funcRef == 0) {
				ShowMessage(importDesc->Name);
					PIMAGE_IMPORT_BY_NAME thunkData = (PIMAGE_IMPORT_BY_NAME) (codeBase + (*thunkRef));
				ShowMessage((LPCSTR)&thunkData->Name);
					result = 0;
					break;
				}
			}
...
HMEMORYMODULE MemoryLoadLibrary(const void *data)
{
	...
	if (dos_header->e_magic != IMAGE_DOS_SIGNATURE) {
		...
		ShowMessage("Not a valid executable file.\n");
	...
	if (old_header->Signature != IMAGE_NT_SIGNATURE) {
		...
		ShowMessage("No PE header found.\n");
    ...
    if (code == NULL) {
		...
		if (code == NULL) {
			...
			ShowMessage("Can't reserve memory");
	...
	if (result->headers->OptionalHeader.AddressOfEntryPoint != 0) {
		...
		if (DllEntry == 0) {
			...
			ShowMessage("Library has no entry point.\n");
		...
		if (!successfull) {
			...
			ShowMessage("Can't attach library.\n");
