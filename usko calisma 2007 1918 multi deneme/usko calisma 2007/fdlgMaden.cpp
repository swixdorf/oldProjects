	...
	: TForm(AOwner)
{
	...
	lItemList[33]	=	ITEM_DC_FLASH;   	bStackable[33] = false;
	lItemList[34]	=	RAINWORM;   		bStackable[34] = false;
	...
	strItemList[33] = "DC Flash";
	strItemList[34] = "Rainworm";
	...
	iTownToInnX[0] = 820;	iTownToInnY[0] = 535;	//Town
...
void ClickAction(int iType, int iItemIndex)
{
	switch( iType )
	{
		case ACT_STORE:
		{
			...
			dlgMaden->lstinn->Checked[iItemIndex] = true;
		...
		case ACT_SELL:
		{
			...
			dlgMaden->lstremove->Checked[iItemIndex] = false;
			dlgMaden->lstnop->Checked[iItemIndex] = false;
			dlgMaden->lstsund->Checked[iItemIndex] = true;
		...
		case ACT_REMOVE:
		{
			...
			dlgMaden->lstsund->Checked[iItemIndex] = false;
			dlgMaden->lstnop->Checked[iItemIndex] = false;
			dlgMaden->lstremove->Checked[iItemIndex] = true;
		...
		case ACT_NOP:
		{
			...
			dlgMaden->lstsund->Checked[iItemIndex] = false;
			dlgMaden->lstremove->Checked[iItemIndex] = false;
			dlgMaden->lstnop->Checked[iItemIndex] = true;
...
bool ConvertCoordinate(TListBox *lst,int index,int *X,int *Y)
{
	if (index>lst->Items->Count)
		return false;
	int virgul,nokta;
	String buf = lst->Items->Strings[index];
	virgul = buf.AnsiPos(",");
	nokta = buf.AnsiPos(":");
	if ((nokta==0)||(virgul==0))
		return false;
	*X	= buf.SubString(2,virgul-2).ToIntDef(0);
	*Y 	= buf.SubString(virgul+1,nokta-virgul-2).ToIntDef(0);
	int rnd = buf.SubString(nokta+1,buf.Length()-nokta).ToIntDef(0);
	if (Random(1)==1)
	{
		*X += Random(rnd);
		if (Random(1)==1)
...
void __fastcall TdlgMaden::tmmadenTimer(TObject *Sender)
{
	...
	if( iCurrentAction == 1 )	//going to mine
	{
		...
		if( !bTowned )
		{
			...
			if( _CMath.fDist(fMyX,fMyY,iTownToInnX[0],iTownToInnY[0]) >= 5.0 )
				Town();
				iLastPos = 0;
			ConvertCoordinate(lstmdncoord,iLastPos,&iNextX,&iNextY);
			/*if( iZone == 21)
			{
				iNextX = iTownToMineX;
				iNextY = iTownToMineY;
		...
		if(Move(iNextX,iNextY,3))
		{
			if(iLastPos == lstmdncoord->Items->Count)
			{
				MadenBekleme = GetTickCount();
	...
	{
		if (GetTickCount()-MadenBekleme<2500)
			return;
	...
	{
		CCharBase pPitman = _CArea.GetNPCByName(1,0,"[Pitman] kursat",false,false,false,50.0,false);
		if(!pPitman.GetBase())
		{
			Town();
			bTownOK = false;
		...
		if( !Move(pPitman.GetFLOAT(KO_POSX),pPitman.GetFLOAT(KO_POSY),3) )	//zzaaaaaaaaa :D :D
			return;
		...
		while(1)
		{
			...
			iLastOreSlot++;
			Sleep(dlgMaden->txtoredelay->Text.ToIntDef(100));
		}
	...
	else if( iCurrentAction == 6 )	//going npc
	{
		...
		if( bInnOpened || Move(iTownToInnX[iLastPos],iTownToInnY[iLastPos],3) )
		{
			if(!bInnOpened)
				iLastPos++;
				...
				for (int i = 0; i < 35; i++)
				{
					...
					if( iItemActionList[i] == ACT_STORE )
					{
						while(1)
						{
						   ...
						   //	frmBot->Memo1->Lines->Add(pInv.GetCurrentSlot());
							if(!pInv.GetBase())
								break;
							if( bStackable[i] )
							{
...
bool CheckInn()
{
	return (bool)(*(BYTE*)(*(DWORD*)(*(DWORD*)(KO_DLGA) + OFF_WAREHOUSE) + 0xFC));
}
...
void __fastcall TdlgMaden::btnblueClick(TObject *Sender)
{
	...
	while(1)
	{
		...
		Sleep(txtoredelay->Text.ToIntDef(1000));
	}
}
//---------------------------------------------------------------------------
...
void InitMineZone()
{
	...
	if( iZone == 21 )
	{
		dlgMaden->lstmdncoord->Items->Clear();
		dlgMaden->lstmdncoord->Items->Add("(810,451):15");
		dlgMaden->lstmdncoord->Items->Add("(776,433):15");
		dlgMaden->lstmdncoord->Items->Add("(667,423):15");
		dlgMaden->lstmdncoord->Items->Add("(640,375):15");
		dlgMaden->iMineX = 640;	dlgMaden->iMineY = 375;
		dlgMaden->iPitmanX = 664;	dlgMaden->iPitmanY = 411;

		dlgMaden->iTownToInnX[0] = 820;	dlgMaden->iTownToInnY[0] = 535;	//Town
	...
	{
		dlgMaden->lstmdncoord->Items->Clear();
		dlgMaden->lstmdncoord->Items->Add("(480,1609):0");
		dlgMaden->lstmdncoord->Items->Add("(517,1564):15");
		dlgMaden->lstmdncoord->Items->Add("(640,1663):15");
		
		dlgMaden->iMineX = 640;	dlgMaden->iMineY = 1663;
		dlgMaden->iPitmanX = 645;	dlgMaden->iPitmanY = 1660;
	...
	{
		dlgMaden->lstmdncoord->Items->Clear();
		dlgMaden->lstmdncoord->Items->Add("(1585,437):7");
		dlgMaden->lstmdncoord->Items->Add("(1540,482):15");
		dlgMaden->lstmdncoord->Items->Add("(1478,403):15");
		

		dlgMaden->iMineX = 1478;	dlgMaden->iMineY = 403;
		dlgMaden->iPitmanX = 1450;	dlgMaden->iPitmanY = 410;
...
void __fastcall TdlgMaden::CheckBox1Click(TObject *Sender)
{
frmBot->bmadenislemci = CheckBox1->Checked;	
}
//---------------------------------------------------------------------------
