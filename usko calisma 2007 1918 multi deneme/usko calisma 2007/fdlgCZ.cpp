...
void __fastcall TdlgCZ::chsuicide2Click(TObject *Sender)
{
	...
	if(bSuicideOnEnemy || bSuicideOnLimit || bSuicideOnMP)
...
void __fastcall TdlgCZ::tmsuicideTimer(TObject *Sender)
{
	...
	*pRevive = 0x12;
	...
	if(bSuicideOnMP)
	{
		DWORD dwMP = _CCharBase.GetDWORD(KO_CRMP);
		DWORD dwMaxMP = _CCharBase.GetDWORD(KO_MXMP);
		if(_CMath.iPerc(dwMaxMP,dwMP) <= (float)iSuicideMPLimit)
		{
			_CPacket.PSend(pDie,3);
			_CPacket.PSend(pRevive,1);
        }
...
void __fastcall TdlgCZ::tmfleeTimer(TObject *Sender)
{
	...
	{
		if(MoveToPos(iFleeX[iFleeLastPos],iFleeY[iFleeLastPos]))
			iFleeLastPos++;
...
void __fastcall TdlgCZ::txtsuicidempChange(TObject *Sender)
{
	iSuicideMPLimit = atoi(txtsuicidemp->Text.c_str());
}
//---------------------------------------------------------------------------

void __fastcall TdlgCZ::chsuicidempClick(TObject *Sender)
{
	bSuicideOnMP = chsuicidemp->Checked;
