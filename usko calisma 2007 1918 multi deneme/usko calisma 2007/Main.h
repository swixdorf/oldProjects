...
#define ITEM_DC_FLASH		399128000
#define ITEM_EXP_FLASH		399127000
...
#define SKILL_DC_FLASH		490146
#define SKILL_EXP_FLASH		490145
...
__inline WORD RWORD(DWORD ulBase)
{
   if (!IsBadReadPtr((VOID*)ulBase, sizeof(DWORD)))
	{
			return(*(WORD*)(ulBase));
...
__inline BYTE RBYTE(DWORD ulBase)
{
   if (!IsBadReadPtr((VOID*)ulBase, sizeof(DWORD)))
	{
			return(*(BYTE*)(ulBase));
   ...
   __inline DWORD WDWORD(DWORD ulBase)
{
   if (!IsBadReadPtr((VOID*)ulBase, sizeof(DWORD)))
	{
			return(*(DWORD*)(ulBase));
...
__inline float Wfloat(DWORD ulBase)
{
   if (!IsBadReadPtr((VOID*)ulBase, sizeof(DWORD)))
	{
			return(*(float*)(ulBase));
...
__inline WORD WWORD(DWORD ulBase)
{
   if (!IsBadReadPtr((VOID*)ulBase, sizeof(DWORD)))
	{
			return(*(WORD*)(ulBase));
...
__inline BYTE WBYTE(DWORD ulBase)
{
   if (!IsBadReadPtr((VOID*)ulBase, sizeof(DWORD)))
	{
			return(*(BYTE*)(ulBase));
...
class CCharBase{
	...
	public:
		...
		char*	GetName();
		inline bool 	IsUPC(){return(*(DWORD*)(dwBase + 0x3c) == 2);};
		...
		if(		GetDWORD(0x3E4)==4
			...
			|| 	GetDWORD(0x3E4)==8095)
			//|| 	GetDWORD(0x0A70)==0
			//|| 	GetDWORD(GetDWORD(0x0A70)+0xa)==3
			//|| 	GetDWORD(GetDWORD(0x0A70)+0xa)==1
			//|| 	GetDWORD(GetDWORD(0x0A70)+0xa)==4
			//|| 	GetDWORD(0x3A0) ==5
			//|| 	GetBYTE(KO_CHST) != 0)
				return false;
			else
		...
		inline bool		IsSit(){return(GetDWORD(0x3E4) == 7);};
...
class CPacket{
	...
	public:
		...
		void PatchHooked();
		BYTE GetIndex();
		inline bool GetConnState(){return *(BYTE*)(*(DWORD*)(KO_SOCK) + 0x40064);};
...
class CArea{
	...
	public:
		...
		//void LootChaosCubes();
		DWORD GetNearstNPCByID(int iIDArray[],int iIDCount,bool bIdType,  bool bAlive, FLOAT fMaxDist);
...
class CInvSlot{
	...
	public:
		...
		void SelectBoots();
		inline DWORD GetBase(){return dwBase;};
		inline int dwItemNum(){
		if(RDWORD(dwBase)) return *(DWORD*)(*(DWORD*)(dwBase + 0x50)) + *(DWORD*)(*(DWORD*)(dwBase + 0x54));
        else return 0;
		};
		inline int iItemN(){
		if(RDWORD(dwBase))return RDWORD(dwBase + 0x58);
		else  return 0;
		};
		inline int iItemDur(){
		if(RDWORD(dwBase))return RDWORD(dwBase + 0x5c);
		else return 0x10000;
		};
		inline char* strItemName(){return (LPSTR)(*(DWORD*)(*(DWORD*)(dwBase + 0x50) + 0xC));};
		inline bool IsFree(){if(dwBase) return false; else return true;};
		inline void SetDurability(int iDur){if(RDWORD(dwBase))*(DWORD*)(dwBase + 0x5c) = iDur;};
...
class CInnSlot{
	...
	public:
		...
		void SelectSlot(int iSlot);
		inline DWORD GetBase(){return dwBase;};
		inline int dwItemNum(){return *(DWORD*)(*(DWORD*)(dwBase + 0x50)) + *(DWORD*)(*(DWORD*)(dwBase + 0x54));};
		inline int iItemN(){return *(DWORD*)(dwBase + 0x58);};
		inline int iItemDur(){return *(DWORD*)(dwBase + 0x5c);};
		inline char* strItemName(){return (LPSTR)(*(DWORD*)(*(DWORD*)(dwBase + 0x50) + 0xC));};
		inline bool IsFree(){if(dwBase) return false; else return true;};
		//Default
		CInnSlot(int iSlot);
...
class CTest{
	public:
	DWORD MySkillBaseCall;
	DWORD MyZPatchCall;
	DWORD MyZPatchCall2;
	DWORD MyZPatchAnaCall;
