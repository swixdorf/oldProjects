...
CCharBase swxselect(float dist);
//CSocket CMainSocket;
DWORD dwINDX = KO_INDX;	//orj adres
bool bOtur = false;
...
BYTE pBuf_BuffSelf1[26] = {0x00,0x01,0x00,0x00,0x00,0x00,0xFF,0xFF,0xFF,0xFF,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
BYTE pBuf_BuffSelf2[26] = {0x00,0x03,0x00,0x00,0x00,0x00,0xFF,0xFF,0xFF,0xFF,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
BYTE pBuf_RemoveSelf[26] = {0x00,0x06,0x00,0x00,0x00,0x00,0xFF,0xFF,0xFF,0xFF,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};

BYTE	pBuf_Attack1[26] = {0x00,0x01,0x00,0x00,0x00,0x00,0xFF,0xFF,0xFF,0xFF,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
...
BYTE	pBuf_Attack3[26] = {0x00,0x03,0x00,0x00,0x00,0x00,0xFF,0xFF,0xFF,0xFF,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
BYTE	pBuf_Attack4[26] = {0x00,0x04,0x00,0x00,0x00,0x00,0xFF,0xFF,0xFF,0xFF,0x00,0x00,0x00,0x00,0x00,0x00,0x9B,0xFF,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};

BYTE	pBuf_FakeSkill[26] = {0x31,0x01,0x00,0x00,0x00,0x00,0xFF,0xFF,0xFF,0xFF,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
...
int WINAPI t41	()
{
	while (1)
	{
	if (GetTickCount()-frmBot->P41Timer>9990)
	{
	frmBot->P41Timer = GetTickCount();
	DWORD Detay41;
	BYTE Komut[7];
	Komut[0]=0x41;
	Komut[1]=1;
	...
	*(DWORD*)&Komut[2]=deger41;//GetTickCount();
	_CPacket.PSend(Komut,sizeof(Komut));
	//Sleep(8000) ;
	}
	LPMSG pMsg;
if(PeekMessageA(pMsg,hGameHWND,0,0,1)){
...
AnsiString BufferToText(char *buf,int size)
{
	String ret;
	for (int i = 0; i < size; i++)
	{
		ret+= IntToHex(buf[i],2);
	}
	return ret;
}
...
BYTE Buffer[1024];
	for (int i = 0; i < len; i++)
	{
		Buffer[i]= StrToInt("0x"+Text.SubString(i*2+1,2));
	}
...
void __fastcall TfrmBot::prepareSkillCodes(TObject *Sender)
{
	...
	*(WORD*)(pBuf_Attack4 + 6) = dwMyID;
	...
	if(iAttackType != 1){	//mage-warrior-priest
		...
		if(iAttackType == 2){	//mage
			...
			if(dwTmpSkillBase)
				lMageCastData62 = *(DWORD*)(dwTmpSkillBase + 0x8c);
		}
	}
}

void __fastcall TfrmBot::prepareForms(TObject *Sender)
{
	//dlgParty = new TdlgParty(NULL);
	Application->CreateForm(__classid(TdlgParty), &dlgParty);
	dlgParty->Hide();

	//dlgBl = new TdlgBl(NULL);
	Application->CreateForm(__classid(TdlgBl), &dlgBl);
	dlgBl->Hide();
	...
	//frmJail = new TfrmJail(NULL);
	Application->CreateForm(__classid(TfrmJail), &frmJail);
	frmJail->Hide();
	...
	dwThreadID_Login		=	GetWindowThreadProcessId( Form4->Handle,			0 );
	dwThreadID_Bot			=	GetWindowThreadProcessId( this->Handle,				0 );
}

void __fastcall TfrmBot::prepareMenu(TObject *Sender)
{
...
void __fastcall TfrmBot::FormShow(TObject *Sender)
{
   ...
   // ShowMessage("amk");
	...
	while (_CCharBase.GetDWORD(KO_MXHP)==0)
	{
		Sleep(1000);
	}
	...
	switch (iAttackType)
	{
		case 1:{
			...
			charchery->Visible		= true;
			switchasas->Visible		= true;
		...
		case 2:
		{
			...
			charchery->Visible		= false;
			grpsuperatak->Visible	= false;
			Button7->Enabled = false;
		}break;
		case 3:
		{
			...
			charchery->Visible		= false;
			grpsuperatak->Visible	= false;
			grppriest->Parent		= grpskill->Parent;
			chpriattack->Parent		= charchery->Parent;
			grppriest->Left			= grpskill->Left;
			grppriest->Top			= grpskill->Top;
			chpriattack->Left		= charchery->Left;
			chpriattack->Top		= charchery->Top;
			chpriattack->BringToFront();
			Button6->Enabled = false;
		}break;
		case 4:
		{
			...
			charchery->Visible		= false;
			grpsuperatak->Visible	= false;
			grpwarrior->Parent		= grpskill->Parent;
			chwarrior->Parent		= charchery->Parent;
			grpwarrior->Left		= grpskill->Left;
			grpwarrior->Top			= grpskill->Top;
			chwarrior->Left			= charchery->Left;
			chwarrior->Top			= charchery->Top;
			chwarrior->BringToFront();
			Button6->Enabled = false;
			Button7->Enabled = false;
		}break;
	default:
	...
	prepareSkillCodes(this);

	this->Caption = "["+ String(_CCharBase.GetName())+"]";
	txtloadsavefile->Text =String(_CCharBase.GetName())+".kole";
	txtsundsave->Text = String(_CCharBase.GetName())+"_sund.kole";
	txtinnsave->Text = String(_CCharBase.GetName())+"_banka.kole";
	txtpotsave->Text = String(_CCharBase.GetName())+"_pot.kole";
	...
	scsh->Position = 2;
	lbshboost->Caption = "2";
	chshowmob->Checked=false;
	bShowMob = false;
	chprisit->Checked = false;
	bSitWhenIdle = false;
	...
	{
		ofstream fWindow;
		fWindow.open("C://wndlist.kole",std::ios_base::out | std::ios_base::app);
...
{
	iSHDelay = txtshdelay->Text.ToIntDef(1000);
	tmsh->Interval = iSHDelay;
	tmfollow->Interval = iSHDelay;
}
//---------------------------------------------------------------------------
...
void __fastcall TfrmBot::tmshTimer(TObject *Sender)
{
	...
	if((!GetAsyncKeyState(VK_LSHIFT))&&!bStickSH){
	_CCharBase.SetFLOAT(KO_SPEED,1);
		return;
    }else{
	_CCharBase.SetFLOAT(KO_SPEED,0);
    }
	if(_CCharBase.IsSit())
		return;
	...
	if(_CMath.fDist(_CCharBase.GetFLOAT(KO_POSX),_CCharBase.GetFLOAT(KO_POSY),fPtrX,fPtrY) > 0.5)
	{
		if((bInterClient && bStickSH))
			SendStickSH();
			SHTeleport(fPtrX,fPtrY,iSHBoost);
...
void __fastcall TfrmBot::tmbuffingTimer(TObject *Sender)
{
	...
	if(dwCurrentTick - iLastPTRefreshTick >= 5000)
		_CParty.Refresh();
		...
		if(dwCurrentTick - iLastWolfTick >= 3000){
			...
			if(!_CCast.IsCasted(dwSkillNum)){
				if(_CBag.FindInvItem(0,ITEM_WOLF) != -1){
					...
					*(BYTE*)(pBuf_BuffSelf1 + 24) = 0x11;

					_CPacket.PSend(pBuf_BuffSelf1,26);
					_CPacket.PSend(pBuf_BuffSelf2,22);

					*(BYTE*)(pBuf_BuffSelf1 + 24) = 0x00;
					*(WORD*)(pBuf_BuffSelf1 + 8) = dwMyID;
					*(WORD*)(pBuf_BuffSelf2 + 8) = dwMyID;
...
//	}
	...
	if(bMage62){
		if(dwCurrentTick - iLastMage62Tick >= 60000){
			...
			if(_CCast.IsCasted(dwSkillNum) == false){
				*(DWORD*)(pBuf_BuffSelf1 + 2) = dwSkillNum;
				*(DWORD*)(pBuf_BuffSelf2 + 2) = dwSkillNum;
				*(BYTE*)(pBuf_BuffSelf1 + 24) = lMageCastData62;
				_CPacket.PSend(pBuf_BuffSelf1,26);
				_CPacket.PSend(pBuf_BuffSelf2,22);
				*(BYTE*)(pBuf_BuffSelf1 + 24) = 0x00;
	...
	if(bTownOnMinPartyMember)
	{
		if(dwCurrentTick - iLastMinPartyTick >= 3000)
		{
			_CParty.Refresh();
			if (_CParty.GetCount()<iMinPartyMember)
			{
				TownAlert();
			}
			iLastMinPartyTick = dwCurrentTick;
		}
	...
	if(bTownOnNoPriest)
	{
		if(dwCurrentTick - iLastMinPartyTick >= 3000)
		{
			...
			{
				int userclass = *(DWORD*)(_CParty.GetUser(i) + KO_PT_CLASS);
				if ((userclass ==111)||(userclass ==112)||(userclass ==104)||
				(userclass ==211)||(userclass ==212)||(userclass ==204)) 
				{
					bprivar=true;	
				}
			...
			iLastTownOnNoPriTick = dwCurrentTick;
		}
...
void __fastcall TfrmBot::btnstartClick(TObject *Sender)
{
	...
	if(iAttackType == 1){	//archer
		...
		dwAsasCastArray[9] = _CCast.MakeCastNum(_CCharBase,"670");
		dwAsasCastArray[10] = _CCast.MakeCastNum(_CCharBase,"675");
		dwAsasCastArray[11] = _CCast.MakeCastNum(_CCharBase,"680");
		...
		for (int i = 0; i < 5; i++) {
			...
			if(dwSkillBase)
				iMageBasicDataArray[i] = *(DWORD*)(dwSkillBase + 0x8c);
		}
		...
		for (int i = 0; i < 21; i++) {
			...
			if(dwSkillBase)
				iMageFlameDataArray[i] = *(DWORD*)(dwSkillBase + 0x8c);

			dwSkillBase = _CTest.GetSkillBase(dwMageIceCastArray[i]);
			if(dwSkillBase)
				iMageIceDataArray[i] = *(DWORD*)(dwSkillBase + 0x8c);

			dwSkillBase = _CTest.GetSkillBase(dwMageLightCastArray[i]);
			if(dwSkillBase)
				iMageLightDataArray[i] = *(DWORD*)(dwSkillBase + 0x8c);
		}
...
void __fastcall TfrmBot::tmattackTimer(TObject *Sender)
{
	...
	CCharBase pTmp = NULL;
	float fDistance = 0, fCenterDistance = 0;
	DWORD TmpVal;
	int i;
	...
	switch(iTargetingType){
		...
		case 3: //sub
		{
			if(iSubTargetingType == 1)
			{
				...
				pTarget = _CArea.GetNearstNPCByID(iTargetMobID,iTargetMobIDCount,bTargetMobIdMob,true,12.5);
			}
...
if(bRouteSH){
SHTeleport((float)iBotCenterX,(float)iBotCenterY,iSHBoost);
}else{
MoveToPos((float)iBotCenterX,(float)iBotCenterY);
}
}
			}
		}
...
MoveToPos((float)iBotCenterX,(float)iBotCenterY);
	...
	if(fDistance > 2.0)
	{
		if(bRunToMob && iBotCenterX && iBotCenterY){
			...
			if(fCenterDistance <= (float)iRunDist){
			if(bRouteSH){
SHTeleport(pTarget.GetFLOAT(KO_POSX),pTarget.GetFLOAT(KO_POSY),iSHBoost);
}else{
MoveToPos(pTarget.GetFLOAT(KO_POSX),pTarget.GetFLOAT(KO_POSY));
}
			}else{
				if(_CMath.fDist(_CCharBase.GetFLOAT(KO_POSX),_CCharBase.GetFLOAT(KO_POSY),(float)iBotCenterX,(float)iBotCenterY) > 0.5 && _CMath.fDist(_CCharBase.GetFLOAT(KO_POSX),_CCharBase.GetFLOAT(KO_POSY),(float)iBotCenterX,(float)iBotCenterY) <= (float)iRunDist){
								if(bRouteSH){
...
MoveToPos((float)iBotCenterX,(float)iBotCenterY);
	...
	if((bCastAble || bTurnBlow) && fDistance <= 12.5)
	{
		if(bShowMob)
...
CAST:
	...
	if(bTurnPercent10){
		...
		*(BYTE*)(pBuf_Attack1 + 24) = 0x10;
		*(BYTE*)(pBuf_Attack3 + 16) = 0x00;
		_CPacket.PSend(pBuf_Attack1,26);
	...
	*(BYTE*)(pBuf_Attack4 + 18) = 0x01;
	...
	if(bTurnBlow && fDistance < 2.7){
		...
		*(WORD*)(pBuf_Attack3 + 8) = wLastTarget;
		*(BYTE*)(pBuf_Attack3 + 16) = 0x00;
		_CPacket.PSend(pBuf_Attack3,26);
		*(BYTE*)(pBuf_Attack3 + 16) = 0x01;
		iLastAttackTick = dwCurrentTick;
	}
			...
			}else
			{
				...
				*(DWORD*)(pBuf_Attack4 + 2) = 108585;
				...
				if(iCurrentCast == 12)	//cs
					*(BYTE*)(pBuf_Attack1 + 24) = 0x0A;

				_CPacket.PSend(pBuf_Attack1,26);
				_CPacket.PSend(pBuf_Attack2,22);
				_CPacket.PSend(pBuf_Attack3,26);
				if(bShowEffect)
					_CPacket.PSend(pBuf_Attack4,26);
			}
...
void CheckFunction()
{
	...
	DWORD dwCallAddr = (Idx * 4) + *(DWORD*)KO_SOCK + 0x4008C;
...
if(_CMath.fDist(fCurX,fCurY,fPtrX,fPtrY) > 0.1){
	...
	*(pBuf+9) = 3;	//moving
	...
	_CPacket.PutWORD(pBuf+10,(int)(OrjX) * 10);
	_CPacket.PutWORD(pBuf+12,(int)(OrjY) * 10);
	_CPacket.PutWORD(pBuf+14,(int)(OrjZ) * 10);
	_CPacket.PSend(pBuf,16);
	return false;
}
else{
	*(pBuf+9) = 0;	//stop
...
bool SHTeleport(float fToX, float fToY, int iBoosting){
	if ((GetTickCount() - frmBot->shWait)<250)
	{
             	float fNewX,fNewY,fNewZ;
				 fNewX= RecvX;
				 fNewY= RecvY;
...
if(_CCharBase.IsSit())
	return false;
if(frmBot->bdied&&frmBot->bdontmoveondie)
	return false;
...
fCurX = _CCharBase.GetFLOAT(KO_POSX);
fCurY = _CCharBase.GetFLOAT(KO_POSY);
fCurZ=_CCharBase.GetFLOAT(KO_POSZ);
...
float fx=fPtrX-fCurX;
float fy=fPtrY-fCurY;
float myNewX,myNewY;
float distance = hypot(fx,fy);

myNewX=(iBoost/distance)*fx;
myNewY=(iBoost/distance)*fy;
WORD fNewX;
WORD fNewY;
if (iBoost <distance) {
fNewX=fCurX+myNewX;
...
fNewY=fPtrY;
	...
	*(pBuf+9) = 3;	//moving
	...
	//dlgAddition->SendOtherClient(pBuf,16);
   	_CPacket.PSend(pBuf,16);
	//Sleep(50);
	return false;
}else{
		...
		if(_CParty.GetCount() >= 2){
			...
			if(dwUser){
				...
				if(wUserID != _CCharBase.GetID()){
					...
					xx = pUser.GetFLOAT(KO_POSX);
					yy = pUser.GetFLOAT(KO_POSY);
					}
					fDist = _CMath.fDist(_CCharBase.GetFLOAT(KO_POSX),_CCharBase.GetFLOAT(KO_POSY),xx,yy);
						if(fDist > 0.5){
							//if(fDist <= 2.0){
								if(bRouteSH){
									SHTeleport(xx,yy,iSHBoost);
								}else{
									MoveToPos(xx,yy);
								}
								//_CCharBase.MoveTo(pUser.GetFLOAT(KO_POSX),pUser.GetFLOAT(KO_POSY));
							//}else{
							//SHTeleport((float)pUser.GetFLOAT(KO_POSX),(float)pUser.GetFLOAT(KO_POSY),iSHBoost);
								/*MoveToPos(pUser.GetFLOAT(KO_POSX),pUser.GetFLOAT(KO_POSY));
				...
				if(fDist > 0.5){
							//if(fDist <= 2.0){
								if(bRouteSH){
									...
									MoveToPos((float)pUser.GetFLOAT(KO_POSX),(float)pUser.GetFLOAT(KO_POSY));
								}
								//_CCharBase.MoveTo(pUser.GetFLOAT(KO_POSX),pUser.GetFLOAT(KO_POSY));
						   //	}else{
						   //	SHTeleport((float)pUser.GetFLOAT(KO_POSX),(float)pUser.GetFLOAT(KO_POSY),iSHBoost);
...
void __fastcall TfrmBot::tmtownTimer(TObject *Sender)
{
	...
	if(bPmControl){
		if (iPmControlCount>9){
			iPmControlCount=0;
			for (int i = 0; i < frmBot->iBlCount; i++) {
				BYTE PmKomut[100];
				memset(PmKomut,0,100);
				BYTE Komut[]={0x35,0x01};
				*(WORD*)&PmKomut[0]=*(WORD*)&Komut[0];
				*(WORD*)&PmKomut[2]=strlen(frmBot->strBlList[i]);
				strcpy(&PmKomut[4],frmBot->strBlList[i]);
				BYTE pSend[1] = {0x00};
				_CPacket.PSend(PmKomut,strlen(frmBot->strBlList[i])+4);
			}
		}
		iPmControlCount++;
	}
	if(bTownOnBl){
		if(_CArea.GetUPCByName(iBlCount,strBlList,NULL,true,NULL,true,false)){
			TownAlert();
			return;
		}
	}
...
void __fastcall TfrmBot::tmsupplyTimer(TObject *Sender)
{
	...
	{
		CInvSlot pItem;
		pItem = _CBag.FindInvItem(0,379099000);
		if(pItem.IsFree())
		{
			chhammer->Checked = false;
		}
		else
		{
			if (GetTickCount() - LastHammerTick > 10000)
			{
				BYTE Komut[] = {0x31,0x03,0xDA,0x7A,0x07,0x00,0xA2,0x07,0xA2,0x07,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,};
				*(WORD*)&Komut[6] = _CCharBase.GetID();
				*(WORD*)&Komut[8] = _CCharBase.GetID();
				_CPacket.PSend(Komut,sizeof(Komut));
                LastHammerTick = GetTickCount();
			}
		}
	...
	if(bSund){
		if(iSundCount){
			...
			if(!bHammer&&(pLItem.GetBase() && iLastDurabilityL != pLItem.iItemDur() &&  pLItem.iItemDur() <= 1000) || (pRItem.GetBase() && iLastDurabilityR != pRItem.iItemDur() && pRItem.iItemDur() <= 1000))
			{
					bSundGo = true;
			}
...
void __fastcall TfrmBot::tmsundgoTimer(TObject *Sender)
{
	...
	if(iSundRoute == 1){	//gidiþ xd
		if(bRouteSH){
			if(SHTeleport(frmBot->iSundX[iSundLastPos],frmBot->iSundY[iSundLastPos],iSHBoost))
				iSundLastPos++;
		}else{
		...
		if(iSundLastPos >= frmBot->iSundCount){
			...
			SundArrived();
			iSundLastPos = frmBot->iSundCount-1;
			iSundRoute = 2;
		}
	}
...
void SundArrived(){
			...
			if(frmBot->bSellPriceLimitAbove || frmBot->bSellPriceLimitLess){
				...
				if(dwItemBase){
					...
					if(*(BYTE*)(dwItemBase + 0x4)){
						dwItemExtBase = _CBag.GetItemExtBase(pItem.dwItemNum(),*(BYTE*)(dwItemBase + 0x4));
						if(dwItemExtBase)
						{
							bExtType = *(BYTE*)(dwItemExtBase + 0x34);
							if(frmBot->chnosell8->Checked)
							{
								if(bExtType != 3)	//craft item olursa nolcak ? :)
								{
									iGrade = *(DWORD*)(dwItemExtBase);
									if((iGrade - ((iGrade / 10) * 10)) == 8)	// +8 mi bu amq ?
										continue;	// +8 satma aga
								}
							}
						}
					}
	...
	bool bwolfaldi=false;
	int AlinanSlot=0;
	if(frmBot->bBuyWolf){
		int iWolfSlot = _CBag.FindInvItem(0,ITEM_WOLF);

		if(iWolfSlot == -1)
			iWolfSlot = _CBag.FindFreeInvSlot(AlinanSlot);
	...
	if(frmBot->bBuyArrow){
		...
		if(iArrowSlot == -1){
		iArrowSlot = _CBag.FindFreeInvSlot(AlinanSlot);
		}
	...
	if(frmBot->bBuyTS){
		...
		if(iTSSlot == -1){
			iTSSlot = _CBag.FindFreeInvSlot(AlinanSlot);
			bScroll = false;
		}
	...
	if(frmBot->bBuyBook){
		...
		if(iBookSlot == -1)
			iBookSlot = _CBag.FindFreeInvSlot(AlinanSlot);
		...
		if(iBookSlot > -1){
			...
			if(iBookN){
				...
				_CPacket.PutBYTE(pSend+15,0x00);
				_CPacket.PutBYTE(pSend+16,0x0B);
				_CPacket.PSend(pSend,17);
				AlinanSlot = iBookSlot + 1;
				Sleep(100);
				RefreshInv();
			Sleep(2000);
			}
	...
	if(frmBot->bBuyRock){
		...
		if(iRockSlot == -1)
			iRockSlot = _CBag.FindFreeInvSlot(AlinanSlot);
	...
	if(frmBot->bBuyMaster){
		...
		if(iMasterSlot == -1)
			iMasterSlot = _CBag.FindFreeInvSlot(AlinanSlot);
...
void __fastcall TfrmBot::tmpotgoTimer(TObject *Sender)
{
	...
	if(iPotRoute == 1){	//gidiþ xd
		if(bRouteSH){
			if(SHTeleport(frmBot->iPotX[iPotLastPos],frmBot->iPotY[iPotLastPos],iSHBoost))
				iPotLastPos++;
		}else{
...
void __fastcall TfrmBot::tminngoTimer(TObject *Sender)
{
	...
	if(iInnRoute == 1){	//gidiþ xd
		if(bRouteSH){
			if(SHTeleport(frmBot->iInnX[iInnLastPos],frmBot->iInnY[iInnLastPos],iSHBoost))
				iInnLastPos++;
		}else{
...
void PotArrived(){
	...
	if(frmBot->bBuyMana){
		...
		if(iPotSlot == -1)
			iPotSlot = _CBag.FindFreeInvSlot(LastBuyPos);
		...
		if(iPotSlot > -1){
			...
			if(iPotN){
				...
				_CPacket.PSend(pSend,17);
				LastBuyPos = iPotSlot + 1 ;
				Sleep(25);
//				ShowMessage("mana aldi");
//				ShowMessage(BufferToHex(pSend,17));
			}
		}
	}
...
//	itoa((int)fMyY,tmp2,10);
	...
	lstsund->Items->Add(IntToStr((int)fMyX)+","+IntToStr((int)fMyY));
//	char buf[16];
//	memset(buf,0,16);
//
//	strcpy(buf,tmp1);
...
void __fastcall TfrmBot::btnaddpotClick(TObject *Sender)
{
	...
	float fMyX = _CCharBase.GetFLOAT(KO_POSX);
	float fMyY = _CCharBase.GetFLOAT(KO_POSY);
	lstpot->Items->Add(IntToStr((int)fMyX)+","+IntToStr((int)fMyY));
...
void __fastcall TfrmBot::btnaddinnClick(TObject *Sender)
{
	...
	float fMyX = _CCharBase.GetFLOAT(KO_POSX);
	float fMyY = _CCharBase.GetFLOAT(KO_POSY);
	lstinn->Items->Add(IntToStr((int)fMyX)+","+IntToStr((int)fMyY));
//	char tmp1[8],tmp2[8];
//	memset(tmp1,0,8);
//	memset(tmp2,0,8);
//
...
{
	MessageBox(0,"Rapor gecici olarak devre disi.",0,0);
	return;
}
//---------------------------------------------------------------------------
...
//				_CPacket.PSend(pSend,1);
//		return;
//_CPacket.PatchHooked() ;return;
//_CParty.Refresh();
//DWORD dwTmpUser = _CParty.GetUser(0);
...
//	float fDistArray[8];
//		dwIDArray[0] = *(WORD*)(dwTmpUser + KO_PT_ID);
//		dwHPArray[0] = *(DWORD*)(dwTmpUser + KO_PT_HP);
//		dwMaxHPArray[0] = *(DWORD*)(dwTmpUser + KO_PT_MAXHP);
//		iClassArray[0] = *(DWORD*)(dwTmpUser + KO_PT_CLASS);
//		bCurseArray[0] = *(BYTE*)(dwTmpUser + KO_PT_CURSE);
//		bDiseaseArray[0] = *(BYTE*)(dwTmpUser + KO_PT_DISEASE);
//		if ((*(DWORD*)(dwTmpUser + KO_PT_NICKL)<16))
//		{
//			strNameArray[0] = (LPSTR)((dwTmpUser + KO_PT_NICK));
//		}
//		else
//		{
//			strNameArray[0] = (LPSTR)(*(DWORD*)(dwTmpUser + KO_PT_NICK));
//		}
//
...
void __fastcall TfrmBot::tmmageattackTimer(TObject *Sender)
{
	...
	bool bRAvailable = (bool)(dwCurrentTick - iLastRAttackTick >= iRTimer);
	bool bCastAble = (bool)(dwCurrentTick - iLastAttackTick >= (DWORD)iAttackDelay);
	bool bBasicTurn[5];
	bool bFlameTurn[17];
	bool bIceTurn[16];
	...
	switch(iTargetingType){
		...
		case 3: //sub
		{
			if(iSubTargetingType == 1)
			{
				...
				pTarget = _CArea.GetNearstNPCByID(iTargetMobID,iTargetMobIDCount,bTargetMobIdMob,true,7.7);
			}
			else if(iSubTargetingType == 2)
			{
				if(!iTargetNickCount)
 ...
 */
pTarget =   swxselect(7.7) ;
SELECT:
	if(pTarget.GetBase() == NULL)
		pTarget.SetBase(pTmp.GetBase());

		// MAGE YARATIGA KOS TEST
...
CAST:
	...
	if(bRAttack && bRAvailable && fDistance < 2.0)
	{
	...
	if(bFlameTurn[16] || bIceTurn[15] || bLightTurn[16]){	//70
		...
		if(dwCurrentCast){
			...
			*(BYTE*)(pBuf_Attack1 + 24) = (BYTE)iCurrentData;
	...
	if(bFlameTurn[19]){	//flame-80 vampiric
		...
		if(dwCurrentCast){
			...
			*(BYTE*)(pBuf_Attack1 + 24) = (BYTE)iMageFlameDataArray[19];
	...
	if(bFlameTurn[20]){	//flame-80 igzination
		...
		if(dwCurrentCast){
			...
			*(BYTE*)(pBuf_Attack1 + 24) = (BYTE)iMageFlameDataArray[20];
...
PARTY:
	...
	for (i = 0; i < iPartyCount; i++) {
		...
		if ((*(DWORD*)(dwTmpUser + KO_PT_NICKL)<16))
		{
			strNameArray[i] = (LPSTR)((dwTmpUser + KO_PT_NICK));
		}
		else
		{
...
bool CheckSelfCure(){
	...
	dwTmp = *(DWORD*)(dwTmp + 0x364);  //bulundu.
	BYTE b = *(BYTE*)(dwTmp + 0x15E);
	if(b)
		return true;
	else
		return false;
...
{
	iPriestAttackType = 2;
	lstpriest->Items->Strings[7]	= "Wrath";
	lstpriest->Items->Strings[8]	= "wield";
	lstpriest->Items->Strings[9]	= "Harsh";
	lstpriest->Items->Strings[10]	= "collapse";
	dwPriestCastArray[2] = _CCast.MakeCastNum(_CCharBase,"611");
	dwPriestCastArray[3] = _CCast.MakeCastNum(_CCharBase,"620");
...
{
	iPriestAttackType = 1;
	lstpriest->Items->Strings[7]	= "collision";
	lstpriest->Items->Strings[8]	= "shuddering";
	lstpriest->Items->Strings[9]	= "ruin";
	lstpriest->Items->Strings[10]	= "Hellish";
	dwPriestCastArray[2] = _CCast.MakeCastNum(_CCharBase,"511");
	dwPriestCastArray[3] = _CCast.MakeCastNum(_CCharBase,"520");
	dwPriestCastArray[4] = _CCast.MakeCastNum(_CCharBase,"542");
	dwPriestCastArray[5] = _CCast.MakeCastNum(_CCharBase,"551");
...
{
	iPriestAttackType = 3;
	lstpriest->Items->Strings[7]	= "tilt";
	lstpriest->Items->Strings[8]	= "Bloody";
	lstpriest->Items->Strings[9]	= "raving edge";
	lstpriest->Items->Strings[10]	= "Hades";
...
{
CCharBase pTarget = NULL;
	switch(frmBot->iTargetingType){

		case 0:	//auto
		{
			if(frmBot->bOnlyEnemy)
				pTarget = _CArea.GetNearstUPC(true,true,dist);
			else
				pTarget = _CArea.GetNearstEnemy(0,true,dist);
			break;
		}

		case 1:	//manuel
		{
			frmBot->wLastTarget = _CCharBase.GetTargetID();
			if(frmBot->wLastTarget == 0xFFFF)
			{
				if(bInterClient)
					SendTarget();
				return pTarget;
			...
			pTarget = _CArea.GetAnyBase(frmBot->wLastTarget);
			if(!pTarget.GetBase())
			{
				if(bInterClient)
				{
					frmBot->wLastTarget = 0xFFFF;
					SendTarget();
				}
				return pTarget;
			}
		...
		case 2:	//list
		{
			...
			pTarget = _CArea.GetNPCByName(frmBot->iMobCount,frmBot->strMobList,NULL,true,true,true,dist,false);
			break;
		}
		...
		case 3: //sub
		{
			if(frmBot->iSubTargetingType == 1)
			{
				...
				pTarget = _CArea.GetNearstNPCByID(frmBot->iTargetMobID,frmBot->iTargetMobIDCount,frmBot->bTargetMobIdMob,true,dist);
			}
			else if(frmBot->iSubTargetingType == 2)
			{
				...
				pTarget = _CArea.GetUPCByName(frmBot->iTargetNickCount,frmBot->strTargetNick,0,true,dist,false,true);
			}
			break;
		}
...
void __fastcall TfrmBot::tmasasbotTimer(TObject *Sender)
{
	...
	bool bTurnBloody = (bool)(bCastAble && bAsasCastArray[9] && dwCurrentTick - iLastAsasCastTick[9] >= 7000);
	bool bTurnBlinding = (bool)(bCastAble && bAsasCastArray[10] && dwCurrentTick - iLastAsasCastTick[10] >= 60000);
	bool bTurnBeast = (bool)(bCastAble && bAsasCastArray[11] && dwCurrentTick - iLastAsasCastTick[11] >= 42000);
	...
	switch(iTargetingType){
		...
		case 3: //sub
		{
			if(iSubTargetingType == 1)
			{
				...
				pTarget = _CArea.GetNearstNPCByID(iTargetMobID,iTargetMobIDCount,bTargetMobIdMob,true,0);
			}
			else if(iSubTargetingType == 2)
			{
				if(!iTargetNickCount)
...
CAST:
	...
	if(bTurnBloody && !bTurnSpike && !bTurnPercent5 && !bTurnPercent10){
		*(DWORD*)(pBuf_Attack3 + 2) = dwAsasCastArray[9];
		*(WORD*)(pBuf_Attack3 + 8) = wLastTarget;
		*(BYTE*)(pBuf_Attack3 + 16) = 0x00;
		_CPacket.PSend(pBuf_Attack3,26);
		iLastAsasAttackTick = dwCurrentTick;
	...
	if(bTurnBlinding && !bTurnBloody && !bTurnSpike && !bTurnPercent5 && !bTurnPercent10){
		*(DWORD*)(pBuf_Attack3 + 2) = dwAsasCastArray[10];
		*(WORD*)(pBuf_Attack3 + 8) = wLastTarget;
		*(BYTE*)(pBuf_Attack3 + 16) = 0x00;
		_CPacket.PSend(pBuf_Attack3,26);
		iLastAsasAttackTick = dwCurrentTick;
	...
	if(bTurnBeast && !bTurnBlinding && !bTurnBloody && !bTurnSpike && !bTurnPercent5 && !bTurnPercent10){
		*(DWORD*)(pBuf_Attack3 + 2) = dwAsasCastArray[11];
		*(WORD*)(pBuf_Attack3 + 8) = wLastTarget;
		*(BYTE*)(pBuf_Attack3 + 16) = 0x00;
		_CPacket.PSend(pBuf_Attack3,26);
		iLastAsasAttackTick = dwCurrentTick;
	...
	if(bTurnCut && !bTurnSpike && !bTurnPercent5 && !bTurnPercent10 && !bTurnBeast && !bTurnBlinding && !bTurnBloody){
	...
	if(bTurnThrust && !bTurnCut && !bTurnSpike && !bTurnPercent5 && !bTurnPercent10 && !bTurnBeast && !bTurnBlinding && !bTurnBloody){
	...
	if(bTurnShock && !bTurnThrust && !bTurnCut && !bTurnSpike && !bTurnPercent5 && !bTurnPercent10 && !bTurnBeast && !bTurnBlinding && !bTurnBloody){
	...
	if(bTurnPierce && !bTurnShock && !bTurnThrust && !bTurnCut && !bTurnSpike && !bTurnPercent5 && !bTurnPercent10 && !bTurnBeast && !bTurnBlinding && !bTurnBloody){
	...
	if(bTurnJab && !bTurnPierce && !bTurnShock && !bTurnThrust && !bTurnCut && !bTurnSpike && !bTurnPercent5 && !bTurnPercent10 && !bTurnBeast && !bTurnBlinding && !bTurnBloody){
	...
	if(bTurnStab2 && !bTurnJab && !bTurnPierce && !bTurnShock && !bTurnThrust && !bTurnCut && !bTurnSpike && !bTurnPercent5 && !bTurnPercent10 && !bTurnBeast && !bTurnBlinding && !bTurnBloody){
	...
	if(bTurnStab && !bTurnStab2 && !bTurnJab && !bTurnPierce && !bTurnShock && !bTurnThrust && !bTurnCut && !bTurnSpike && !bTurnPercent5 && !bTurnPercent10 && !bTurnBeast && !bTurnBlinding && !bTurnBloody){
	...
	if(bTurnStroke && !bTurnStab && !bTurnStab2 && !bTurnJab && !bTurnPierce && !bTurnShock && !bTurnThrust && !bTurnCut && !bTurnSpike && !bTurnPercent5 && !bTurnPercent10 && !bTurnBeast && !bTurnBlinding && !bTurnBloody){
...
void CastRock(DWORD ID){
	...
	*(BYTE*)(pBuf_Attack1 + 24) = 0x05;

	_CPacket.PSend(pBuf_Attack1,26);
	_CPacket.PSend(pBuf_Attack2,22);
	_CPacket.PSend(pBuf_Attack3,26);

	*(BYTE*)(pBuf_Attack1 + 24) = 0x00;
}
...
{
	bLightStrike	= lstpriest->Checked[0];
	bBrightness		= lstpriest->Checked[1];
	bLightAttack	= lstpriest->Checked[2];
	bLightCounter	= lstpriest->Checked[3];
	bCriticalLight	= lstpriest->Checked[4];

	chpriattack->Enabled=true;
	for (int i = 5; i < 13; i++) {
		bPriestCastArray[i-5] = lstpriest->Checked[i];
	}
}
//---------------------------------------------------------------------------
...
if(frmBot->bdied&&frmBot->bdontmoveondie)
	...
	if(_CMath.fDist(fMyPosX,fMyPosY,fX,fY) > 0.5){
...
//	fSave << tmp << "\n";
	...
	itoa(dlgBl->chbldc->Checked,tmp,10);
	fSave << tmp << "\n";

	itoa(dlgBl->chbltown->Checked,tmp,10);
	fSave << tmp << "\n";

	itoa(frmBot->iBlCount,tmp,10);
	fSave << tmp << "\n";

	if(frmBot->iBlCount){
		for (int i = 0; i < frmBot->iBlCount; i++) {
		   fSave << dlgBl->lstBladd->Items->operator [](i).c_str() << "\n";
		}
	}

	itoa(frmBot->bAsasCastArray[9],tmp,10);
	fSave << tmp << "\n";
	itoa(frmBot->bAsasCastArray[10],tmp,10);
	fSave << tmp << "\n";
	itoa(frmBot->bAsasCastArray[11],tmp,10);
	fSave << tmp << "\n";

	fSave << dlgMaden->txtoredelay->Text.c_str() << "\n";

	itoa(frmBot->bdontmoveondie,tmp,10);
	fSave << tmp << "\n";

		itoa(frmBot->bFlashDc,tmp,10);
	fSave << tmp << "\n";

		itoa(frmBot->bFlashExp,tmp,10);
	fSave << tmp << "\n";
...
/*
	...
	if(frmBot->txtloadsavefile->Text.Length() == 0)
		return;
	TIniFile *saveini;
	saveini = new TIniFile(frmBot->txtloadsavefile->Text);
		saveini->WriteInteger	("Ayar"	,	"iSHBoost"					,	frmBot->iSHBoost) ;
		saveini->WriteInteger	("Ayar"	,	"iSHDelay"					,	frmBot->iSHDelay) ;
		saveini->WriteBool		("Ayar"	,	"bRouteSH"					,	frmBot->bRouteSH) ;
		saveini->WriteBool		("Ayar"	,	"bArcheryAttack"			,	frmBot->bArcheryAttack) ;
	for (int i = 0; i < 20; i++)
	{
		saveini->WriteBool		("Ayar"	,	"bArcheryCastArray"+IntToStr(i),frmBot->bArcheryCastArray[i]) ;
	}
		saveini->WriteBool		("Ayar"	,	"bBlow"						,	frmBot->bBlow) ;
		saveini->WriteBool		("Ayar"	,	"bBonus3_1"					,	frmBot->bBonus3_1) ;
		saveini->WriteInteger	("Ayar"	,	"iTargetingType"			,	frmBot->iTargetingType) ;
		saveini->WriteInteger	("Ayar"	,	"iTargetingType"			,	frmBot->iMobCount) ;
	if(frmBot->iMobCount)
	{
		for (int i = 0; i < frmBot->iMobCount; i++)
		{
		saveini->WriteString	("Ayar"	,	"lstatakmobs"+IntToStr(i)	,	frmBot->lstatakmobs->Items->Strings[i]) ;
		}
	}
		saveini->WriteInteger	("Ayar"	,	"iAttackDelay"				,	frmBot->iAttackDelay) ;
		saveini->WriteBool		("Ayar"	,	"bShowEffect"				,	frmBot->bShowEffect) ;
		saveini->WriteBool		("Ayar"	,	"bShowMob"					,	frmBot->bShowMob) ;
		saveini->WriteBool		("Ayar"	,	"bWaitForKill"			   	,	frmBot->bWaitForKill) ;
		saveini->WriteBool		("Ayar"	,	"bOnlyEnemy"				,	frmBot->bOnlyEnemy) ;
		saveini->WriteBool		("Ayar"	,	"bOnlyTarget"				,	frmBot->bOnlyTarget) ;
		saveini->WriteBool		("Ayar"	,	"bBonus5"					,	frmBot->bBonus3) ;
		saveini->WriteInteger	("Ayar"	,	"iBonusDistance"			,	frmBot->iBonusDistance) ;
		saveini->WriteBool		("Ayar"	,	"chstealthhack"				,	frmBot->chstealthhack->Checked) ;
		saveini->WriteBool		("Ayar"	,	"bAutoRevive"				,	frmBot->bAutoRevive) ;
		saveini->WriteBool		("Ayar"	,	"bMinor"					,	frmBot->bMinor) ;
		saveini->WriteInteger	("Ayar"	,	"iMinorLimit"				,	frmBot->iMinorLimit) ;
		saveini->WriteBool		("Ayar"	,	"bWolf"						,	frmBot->bWolf) ;
		saveini->WriteBool		("Ayar"	,	"bTS"						,	frmBot->bTS) ;
		saveini->WriteBool		("Ayar"	,	"bMana"						,	frmBot->bMana) ;
		saveini->WriteInteger	("Ayar"	,	"iManaType"					,	frmBot->iManaType) ;
		saveini->WriteInteger	("Ayar"	,	"iManaLimit"				,	frmBot->iManaLimit) ;
		saveini->WriteBool		("Ayar"	,	"bHP"						,	frmBot->bHP) ;
		saveini->WriteInteger	("Ayar"	,	"iHPType"					,	frmBot->iHPType) ;
		saveini->WriteInteger	("Ayar"	,	"iHPLimit"					,	frmBot->iHPLimit) ;
		saveini->WriteBool		("Ayar"	,	"bSprint"					,	frmBot->bSprint) ;
		saveini->WriteBool		("Ayar"	,	"bHide"						,	frmBot->bHide) ;
		saveini->WriteBool		("Ayar"	,	"bScaled"					,	frmBot->bScaled) ;
		saveini->WriteBool		("Ayar"	,	"bSW"						,	frmBot->bSW) ;
		saveini->WriteBool		("Ayar"	,	"bEvade"					,	frmBot->bEvade) ;
		saveini->WriteBool		("Ayar"	,	"bLup"						,	frmBot->bLup) ;
		saveini->WriteBool		("Ayar"	,	"bAntiFreeze"				,	frmBot->bAntiFreeze) ;
		saveini->WriteBool		("Ayar"	,	"bLF"						,	frmBot->bLF) ;
		saveini->WriteBool		("Ayar"	,	"bSafety"					,	frmBot->bSafety) ;
		saveini->WriteBool		("Ayar"	,	"bCats"						,	frmBot->bCats) ;
		saveini->WriteBool		("Ayar"	,	"bMage62"					,	frmBot->bMage62) ;
		saveini->WriteBool		("Ayar"	,	"bCure"						,	frmBot->bCure) ;
		saveini->WriteBool		("Ayar"	,	"bMoving"					,	frmBot->bMoving) ;
		saveini->WriteInteger	("Ayar"	,	"iMoveCount"				,	frmBot->iMoveCount) ;
	if(frmBot->iMoveCount)
	{
		for (int i = 0; i < frmBot->iMoveCount; i++)
		{
		saveini->WriteString	("Ayar"	,	"lstmove"+IntToStr(i)		,	frmBot->lstmove->Items->Strings[i]) ;
		}
	}
		saveini->WriteString	("Ayar"	,	"txtchat"					,	frmBot->txtchat->Text) ;
		saveini->WriteInteger	("Ayar"	,	"iChatDelay"				,	frmBot->iChatDelay) ;
		saveini->WriteInteger	("Ayar"	,	"iChatType"					,	frmBot->iChatType) ;
		saveini->WriteBool		("Ayar"	,	"chchat"					,	frmBot->chchat->Checked) ;
		saveini->WriteBool		("Ayar"	,	"bLoot"						,	frmBot->bLoot) ;
		saveini->WriteBool		("Ayar"	,	"bLootCoins"				,	frmBot->bLootCoins) ;
		saveini->WriteBool		("Ayar"	,	"bMoveToChest"				,	frmBot->bMoveToChest) ;
		saveini->WriteBool		("Ayar"	,	"bStunChar"					,	frmBot->bStunChar) ;
		saveini->WriteInteger	("Ayar"	,	"iMaxChestDistance"			,	frmBot->iMaxChestDistance) ;
		saveini->WriteInteger	("Ayar"	,	"iCenterX"					,	frmBot->iCenterX) ;
		saveini->WriteInteger	("Ayar"	,	"iCenterY"					,	frmBot->iCenterY) ;
		saveini->WriteBool		("Ayar"	,	"bLootUnique"				,	frmBot->bLootUnique) ;
		saveini->WriteBool		("Ayar"	,	"bLootUpgrade"				,	frmBot->bLootUpgrade) ;
		saveini->WriteInteger	("Ayar"	,	"iLootUpgradeLimit"			,	frmBot->iLootUpgradeLimit) ;
		saveini->WriteBool		("Ayar"	,	"bLootList"					,	frmBot->bLootList) ;
		saveini->WriteBool		("Ayar"	,	"bLootPriceLimit"			,	frmBot->bLootPriceLimit) ;
		saveini->WriteInteger	("Ayar"	,	"iLootPriceLimit"			,	frmBot->iLootPriceLimit) ;
		saveini->WriteInteger	("Ayar"	,	"iLootCount"				,	frmBot->iLootCount) ;
	if(frmBot->iLootCount)
	{
		for (int i = 0; i < frmBot->iLootCount; i++)
		{
		saveini->WriteString	("Ayar"	,	"lstlootids"+IntToStr(i)	,	frmBot->lstlootids->Items->Strings[i]) ;
		}
	}
		saveini->WriteBool		("Ayar"	,	"bSund"						,	frmBot->bSund) ;
		saveini->WriteBool		("Ayar"	,	"bPot"						,	frmBot->bPot) ;
		saveini->WriteBool		("Ayar"	,	"bInn"						,	frmBot->bInn) ;
		saveini->WriteBool		("Ayar"	,	"bBuyMana"					,	frmBot->bBuyMana) ;
		saveini->WriteBool		("Ayar"	,	"bBuyHP"					,	frmBot->bBuyHP) ;
		saveini->WriteInteger	("Ayar"	,	"iBuyManaLimit"				,	frmBot->iBuyManaLimit) ;
		saveini->WriteInteger	("Ayar"	,	"iBuyHPLimit"				,	frmBot->iBuyHPLimit) ;
		saveini->WriteBool		("Ayar"	,	"bInnDepot"					,	frmBot->bInnDepot) ;
		saveini->WriteInteger	("Ayar"	,	"iInnBeginSlot"				,	frmBot->iInnBeginSlot) ;
		saveini->WriteBool		("Ayar"	,	"bAutoSell"					,	frmBot->bAutoSell) ;
		saveini->WriteBool		("Ayar"	,	"bSellSlotLimit"			,	frmBot->bSellSlotLimit) ;
		saveini->WriteInteger	("Ayar"	,	"iSellBeginSlot"			,	frmBot->iSellBeginSlot) ;
		saveini->WriteBool		("Ayar"	,	"bSellPriceLimitAbove"		,	frmBot->bSellPriceLimitAbove) ;
		saveini->WriteBool		("Ayar"	,	"bSellPriceLimitLess"		,	frmBot->bSellPriceLimitLess) ;
		saveini->WriteInteger	("Ayar"	,	"iSellPriceLimitAbove"		,	frmBot->iSellPriceLimitAbove) ;
		saveini->WriteInteger	("Ayar"	,	"iSellPriceLimitLess"		,	frmBot->iSellPriceLimitLess) ;
		saveini->WriteBool		("Ayar"	,	"bSellWaitForRPR"			,	frmBot->bSellWaitForRPR) ;
		saveini->WriteBool		("Ayar"	,	"bBuyWolf"					,	frmBot->bBuyWolf) ;
		saveini->WriteBool		("Ayar"	,	"bBuyArrow"					,	frmBot->bBuyArrow) ;
		saveini->WriteBool		("Ayar"	,	"bBuyTS"					,	frmBot->bBuyTS) ;
		saveini->WriteBool		("Ayar"	,	"bBuyBook"					,	frmBot->bBuyBook) ;
		saveini->WriteBool		("Ayar"	,	"bBuyRock"					,	frmBot->bBuyRock) ;
		saveini->WriteInteger	("Ayar"	,	"iBuyTS"					,	frmBot->iBuyTS) ;
		saveini->WriteInteger	("Ayar"	,	"iBuyBook"					,	frmBot->iBuyBook) ;
		saveini->WriteInteger	("Ayar"	,	"iBuyRock"					,	frmBot->iBuyRock) ;
		saveini->WriteBool		("Ayar"	,	"chwh"						,	frmBot->chwh->Checked) ;
		saveini->WriteBool		("Ayar"	,	"bFollowLeader"				,	frmBot->bFollowLeader) ;
		saveini->WriteBool		("Ayar"	,	"bFollowSelected"			,	frmBot->bFollowSelected) ;
		saveini->WriteBool		("Ayar"	,	"bAutoParty"				,	frmBot->bAutoParty) ;
		saveini->WriteBool		("Ayar"	,	"bInformAfterSupply"		,	frmBot->bInformAfterSupply) ;
		saveini->WriteString	("Ayar"	,	"lstlootids"					,	frmBot->txtinformsupply->Text) ;
		saveini->WriteBool		("Ayar"	,	"bAutoInvite"				,	frmBot->bAutoInvite) ;
		saveini->WriteString	("Ayar"	,	"txtautoadd"				,	dlgParty->txtautoadd->Text) ;
		saveini->WriteBool		("Ayar"	,	"bTownOnGM"					,	frmBot->bTownOnGM) ;
		saveini->WriteBool		("Ayar"	,	"bTownOnMob"				,	frmBot->bTownOnMob) ;
		saveini->WriteBool		("Ayar"	,	"bTownOnNoTS"				,	frmBot->bTownOnNoTS) ;
		saveini->WriteBool		("Ayar"	,	"bTownOnNoArrow"			,	frmBot->bTownOnNoArrow) ;
		saveini->WriteBool		("Ayar"	,	"bTownOnNoMana"				,	frmBot->bTownOnNoMana) ;
		saveini->WriteBool		("Ayar"	,	"bTownOnNoParty"			,	frmBot->bTownOnNoParty) ;
		saveini->WriteBool		("Ayar"	,	"bRingOnDie"				,	frmBot->bRingOnDie) ;
		saveini->WriteBool		("Ayar"	,	"bRingOnTown"				,	frmBot->bRingOnTown) ;
		saveini->WriteInteger	("Ayar"	,	"iAlertMobCount"			,	frmBot->iAlertMobCount) ;
	if(frmBot->iAlertMobCount)
	{
		for (int i = 0; i < frmBot->iAlertMobCount; i++)
		{
		saveini->WriteString	("Ayar"	,	"lsttownmobs"+IntToStr(i)	,	frmBot->lsttownmobs->Items->Strings[i]) ;
		}
	}
		saveini->WriteBool		("Ayar"	,	"chtargetsize"				,	frmBot->chtargetsize->Checked) ;
		saveini->WriteBool		("Ayar"	,	"bChatTP"					,	frmBot->bChatTP) ;
		saveini->WriteBool		("Ayar"	,	"bLimitTP"					,	frmBot->bLimitTP) ;
		saveini->WriteBool		("Ayar"	,	"bAutoGate"					,	frmBot->bAutoGate) ;
		saveini->WriteString("Ayar"	,	"txttpchat"						,	frmBot->txttpchat->Text) ;
		saveini->WriteInteger	("Ayar"	,	"iTPLimit"					,	frmBot->iTPLimit) ;
		saveini->WriteInteger	("Ayar"	,	"iGateLimit"				,	frmBot->iGateLimit) ;
		saveini->WriteBool		("Ayar"	,	"bMageAttack"				,	frmBot->bMageAttack) ;
	for (int i = 0; i < 5; i++)
	{
		saveini->WriteBool		("Ayar"	,	"bMageBasicCastArray"+IntToStr(i),frmBot->bMageBasicCastArray[i]) ;
	}
	for (int i = 0; i < 17; i++)
	{
		saveini->WriteBool		("Ayar"	,	"bMageBasicCastArray"+IntToStr(i),frmBot->bMageFlameCastArray[i]) ;
	}
	for (int i = 0; i < 16; i++)
	{
		saveini->WriteBool		("Ayar"	,	"bMageIceCastArray"+IntToStr(i),frmBot->bMageIceCastArray[i]) ;
	}
	for (int i = 0; i < 17; i++)
	{
		saveini->WriteBool		("Ayar"	,	"bMageLightCastArray"+IntToStr(i),frmBot->bMageLightCastArray[i]) ;
	}
	for (int i = 0; i < 8; i++)
	{
		saveini->WriteBool		("Ayar"	,	"bPriestMembers"+IntToStr(i),frmBot->bPriestMembers[i]) ;
	}
		saveini->WriteBool		("Ayar"	,	"bPriestHeal"	,	frmBot->bPriestHeal) ;
		saveini->WriteInteger	("Ayar"	,	"iHealType"	,	frmBot->iHealType) ;
		saveini->WriteInteger	("Ayar"	,	"iHealCastIndex"	,	frmBot->iHealCastIndex) ;
		saveini->WriteInteger	("Ayar"	,	"iHealLimit"	,	frmBot->iHealLimit) ;
		saveini->WriteBool		("Ayar"	,	"bHealLessHPFirst"	,	frmBot->bHealLessHPFirst) ;
		saveini->WriteBool		("Ayar"	,	"bBuff"	,	frmBot->bBuff) ;
		saveini->WriteBool		("Ayar"	,	"bAC"	,	frmBot->bAC) ;
		saveini->WriteBool		("Ayar"	,	"bResist"	,	frmBot->bResist) ;
		saveini->WriteInteger	("Ayar"	,	"iBuffCastIndex"	,	frmBot->iBuffCastIndex) ;
		saveini->WriteBool		("Ayar"	,	"bUndyForWarriors"	,	frmBot->bUndyForWarriors) ;
		saveini->WriteInteger	("Ayar"	,	"iACCastIndex"	,	frmBot->iACCastIndex) ;
		saveini->WriteInteger	("Ayar"	,	"iResistCastIndex"	,	frmBot->iResistCastIndex) ;
		saveini->WriteBool		("Ayar"	,	"bGroupHeal"	,	frmBot->bGroupHeal) ;
		saveini->WriteBool		("Ayar"	,	"bRestore"	,	frmBot->bRestore) ;
		saveini->WriteInteger	("Ayar"	,	"iGroupCastIndex"	,	frmBot->iGroupCastIndex) ;
		saveini->WriteInteger	("Ayar"	,	"iRestoreCastIndex"	,	frmBot->iRestoreCastIndex) ;
		saveini->WriteBool		("Ayar"	,	"bDebuff"	,	frmBot->bDebuff) ;
		saveini->WriteBool		("Ayar"	,	"bMalice"	,	frmBot->bMalice) ;
		saveini->WriteBool		("Ayar"	,	"bParasite"	,	frmBot->bParasite) ;
		saveini->WriteBool		("Ayar"	,	"bTorment"	,	frmBot->bTorment) ;
		saveini->WriteBool		("Ayar"	,	"bMassive"	,	frmBot->bMassive) ;
		saveini->WriteBool		("Ayar"	,	"bChatHeal"	,	frmBot->bChatHeal) ;
		saveini->WriteBool		("Ayar"	,	"bChatBuff"	,	frmBot->bChatBuff) ;
		saveini->WriteString("Ayar"	,	"txtchatheal"	,	frmBot->txtchatheal->Text) ;
		saveini->WriteString("Ayar"	,	"txtchatbuff"	,	frmBot->txtchatbuff->Text) ;
		saveini->WriteBool		("Ayar"	,	"bSitWhenIdle"	,	frmBot->bSitWhenIdle) ;
		saveini->WriteBool		("Ayar"	,	"bPriAutoCure"	,	frmBot->bPriAutoCure) ;
		saveini->WriteBool		("Ayar"	,	"bPriAutoSTR"	,	frmBot->bPriAutoSTR) ;
		saveini->WriteBool		("Ayar"	,	"bPriAutoBook"	,	frmBot->bPriAutoBook) ;
		saveini->WriteBool		("Ayar"	,	"bPriPrintName"	,	frmBot->bPriPrintName) ;
		saveini->WriteBool		("Ayar"	,	"bRAttack"	,	frmBot->bRAttack) ;
		saveini->WriteBool		("Ayar"	,	"bRunToMob"	,	frmBot->bRunToMob) ;
		saveini->WriteBool		("Ayar"	,	"bRunToMob"	,	frmBot->bRunToMob) ;
		saveini->WriteInteger	("Ayar"	,	"iRunDist"	,	frmBot->iRunDist) ;
		saveini->WriteBool		("Ayar"	,	"bRock"	,	frmBot->bRock) ;
		saveini->WriteBool		("Ayar"	,	"bAsasAttack"	,	frmBot->bAsasAttack) ;
	for (int i = 0; i < 9; i++)
	{
		saveini->WriteBool		("Ayar"	,	"bAsasCastArray"+IntToStr(i),frmBot->bAsasCastArray[i]) ;
	}
		saveini->WriteBool		("Ayar"	,	"bAsasPercent5"	,	frmBot->bAsasPercent5) ;
		saveini->WriteBool		("Ayar"	,	"bAsasPercent10"	,	frmBot->bAsasPercent10) ;
		saveini->WriteBool		("Ayar"	,	"bWarriorAttack"	,	frmBot->bWarriorAttack) ;
		saveini->WriteInteger	("Ayar"	,	"iWarriorCastIndex"	,	frmBot->iWarriorCastIndex) ;
		saveini->WriteBool		("Ayar"	,	"bBinding"	,	frmBot->bBinding) ;
		saveini->WriteBool		("Ayar"	,	"bProvoke"	,	frmBot->bProvoke) ;
		saveini->WriteBool		("Ayar"	,	"bSprintWarrior"	,	frmBot->bSprintWarrior) ;
		saveini->WriteBool		("Ayar"	,	"bDefenseWarrior"	,	frmBot->bDefenseWarrior) ;
		saveini->WriteBool		("Ayar"	,	"bGainWarrior"	,	frmBot->bGainWarrior) ;
		saveini->WriteBool		("Ayar"	,	"bPriestAttack"	,	frmBot->bPriestAttack) ;
		saveini->WriteInteger	("Ayar"	,	"iPriestAttackType"	,	frmBot->iPriestAttackType) ;
	for (int i = 0; i < 8; i++)
	{
		saveini->WriteBool		("Ayar"	,	"bPriestCastArray"+IntToStr(i),frmBot->bPriestCastArray[i]) ;
	}
		saveini->WriteBool		("Ayar"	,	"bLightStrike"	,	frmBot->bLightStrike) ;
		saveini->WriteBool		("Ayar"	,	"bLightCounter"	,	frmBot->bLightCounter) ;
		saveini->WriteBool		("Ayar"	,	"bBrightness"	,	frmBot->bBrightness) ;
		saveini->WriteBool		("Ayar"	,	"bCriticalLight"	,	frmBot->bCriticalLight) ;
		saveini->WriteBool		("Ayar"	,	"bLightAttack"	,	frmBot->bLightAttack) ;
		saveini->WriteBool		("Ayar"	,	"bSTR30"	,	frmBot->bSTR30) ;
		saveini->WriteBool		("Ayar"	,	"bFarChest"	,	frmBot->bFarChest) ;
		saveini->WriteBool		("Ayar"	,	"bRingOnDC"	,	frmBot->bRingOnDC) ;
		saveini->WriteBool		("Ayar"	,	"bCloseOnDC"	,	frmBot->bCloseOnDC) ;
		saveini->WriteBool		("Ayar"	,	"bBuyMaster"	,	frmBot->bBuyMaster) ;
		saveini->WriteInteger	("Ayar"	,	"iBuyMaster"	,	frmBot->iBuyMaster) ;
		saveini->WriteBool		("Ayar"	,	"chautoadd"	,	frmBot->chautoadd->Checked) ;
		saveini->WriteInteger	("Ayar"	,	"iAddCount"	,	frmBot->iAddCount) ;
	if(frmBot->iAddCount)
	{
		for (int i = 0; i < frmBot->iAddCount; i++)
		{
		saveini->WriteString("Ayar"	,	"lstpartyadd"+IntToStr(i),dlgParty->lstpartyadd->Items->Strings[i]) ;
		}
	}
		saveini->WriteBool		("Ayar"	,	"chautoadd"	,	frmBot->bNoTrade) ;
		saveini->WriteBool		("Ayar"	,	"bNoTrade"	,	frmBot->bNoTrade) ;
		saveini->WriteBool		("Ayar"	,	"bNoParty"	,	frmBot->bNoParty) ;
		saveini->WriteBool		("Ayar"	,	"bNoCombat"	,	frmBot->bNoCombat) ;
		saveini->WriteBool		("Ayar"	,	"bSuicideOnEnemy"	,	dlgCZ->bSuicideOnEnemy) ;
		saveini->WriteBool		("Ayar"	,	"bSuicideOnLimit"	,	dlgCZ->bSuicideOnLimit) ;
		saveini->WriteBool		("Ayar"	,	"bStopBotOnNoPT"	,	dlgCZ->bStopBotOnNoPT) ;
		saveini->WriteBool		("Ayar"	,	"bLeavePTOnDie"	,	dlgCZ->bLeavePTOnDie) ;
		saveini->WriteBool		("Ayar"	,	"bGetInnHP"	,	frmBot->bGetInnHP) ;
		saveini->WriteBool		("Ayar"	,	"bGetInnMP"	,	frmBot->bGetInnMP) ;
		saveini->WriteInteger	("Ayar"	,	"iGetInnHP"	,	frmBot->iGetInnHP) ;
		saveini->WriteInteger	("Ayar"	,	"iGetInnMP"	,	frmBot->iGetInnMP) ;
		saveini->WriteBool		("Ayar"	,	"bLegalAttack"	,	frmBot->bLegalAttack) ;
		saveini->WriteBool		("Ayar"	,	"bAccept"	,	dlgParty->bAccept) ;
		saveini->WriteBool		("Ayar"	,	"bRequest"	,	dlgParty->bRequest) ;
		saveini->WriteBool		("Ayar"	,	"bGetInnMP"	,	frmBot->bGetInnMP) ;
		saveini->WriteBool		("Ayar"	,	"bPartySW"	,	frmBot->bPartySW) ;
		saveini->WriteBool		("Ayar"	,	"bStartInPT"	,	dlgCZ->bStartInPT) ;
		saveini->WriteInteger	("Ayar"	,	"iSleepRatio"	,	frmBot->iSleepRatio) ;
		saveini->WriteBool		("Ayar"	,	"bSaveCPU"	,	frmBot->bSaveCPU) ;
		saveini->WriteBool		("Ayar"	,	"bFlee"	,	dlgCZ->bFlee) ;
		saveini->WriteBool		("Ayar"	,	"bFleeTown"	,	dlgCZ->bFleeTown) ;
		saveini->WriteBool		("Ayar"	,	"bFleeRing"	,	dlgCZ->bFleeRing) ;
		saveini->WriteInteger	("Ayar"	,	"iFleeCount"	,	dlgCZ->iFleeCount) ;
	if(dlgCZ->iFleeCount)
	{
		for (int i = 0; i < dlgCZ->iFleeCount; i++)
		{
		saveini->WriteString("Ayar"	,	"lstflee"+IntToStr(i),dlgCZ->lstflee->Items->Strings[i]) ;
		}
	}
	for (int i = 17; i < 21; i++)
	{
		saveini->WriteBool		("Ayar"	,	"bMageFlameCastArray"+IntToStr(i),frmBot->bMageFlameCastArray[i]) ;
	}
	for (int i = 16; i < 21; i++)
	{
		saveini->WriteBool		("Ayar"	,	"bMageIceCastArray"+IntToStr(i),frmBot->bMageIceCastArray[i]) ;
	...
	for (int i = 17; i < 21; i++)
	{
		...
		saveini->WriteBool		("Ayar"	,	"bKSMod"	,	frmBot->bKSMod) ;
		saveini->WriteBool		("Ayar"	,	"bAutoMeat"	,	frmBot->bAutoMeat) ;
		saveini->WriteInteger	("Ayar"	,	"iTargetMobIDCount"	,	frmBot->iTargetMobIDCount) ;

	if(frmBot->iTargetMobIDCount)
	{
		for (int i = 0; i < frmBot->iTargetMobIDCount; i++)
		{
		saveini->WriteString("Ayar"	,	"lstidmob"+IntToStr(i),frmBot->lstidmob->Items->Strings[i]) ;
		}
	}
		saveini->WriteInteger	("Ayar"	,	"iTargetNickCount"	,	frmBot->iTargetNickCount) ;
	if(frmBot->iTargetNickCount)
	{
		for (int i = 0; i < frmBot->iTargetNickCount; i++)
		{
		saveini->WriteString("Ayar"	,	"lstnickmob"+IntToStr(i),frmBot->lstnickmob->Items->Strings[i]) ;
		}
	}
		saveini->WriteInteger	("Ayar"	,	"iSubTargetingType"	,	frmBot->iSubTargetingType) ;
		saveini->WriteBool		("Ayar"	,	"bSH"	,	dlgMaden->bSH) ;
		saveini->WriteBool		("Ayar"	,	"chbldc"	,	dlgBl->chbldc->Checked) ;
		saveini->WriteBool		("Ayar"	,	"chbltown"	,	dlgBl->chbltown->Checked) ;
		saveini->WriteInteger	("Ayar"	,	"iBlCount"	,	frmBot->iBlCount) ;
	if(frmBot->iBlCount)
	{
		for (int i = 0; i < frmBot->iBlCount; i++)
		{
		saveini->WriteString("Ayar"	,	"lstBladd"+IntToStr(i),dlgBl->lstBladd->Items->Strings[i]) ;
		}
	}
		saveini->WriteBool		("Ayar"	,	"bAsasCastArray9"	,	frmBot->bAsasCastArray[9]) ;
		saveini->WriteBool		("Ayar"	,	"bAsasCastArray10"	,	frmBot->bAsasCastArray[10]) ;
		saveini->WriteBool		("Ayar"	,	"bAsasCastArray11"	,	frmBot->bAsasCastArray[11]) ;
		saveini->WriteString("Ayar"	,	"txtoredelay"	,	dlgMaden->txtoredelay->Text) ;
...
//	frmBot->chtargetsizeClick(NULL);	//init
	...
	for (int i = 5; i < 13; i++) {
		bRet = fLoad.getline(Buffer,128);
		if(bRet == false || strlen(Buffer) == 0)	return;
		frmBot->lstpriest->Checked[i] = atoi(Buffer)-5;
		frmBot->bPriestCastArray[i] = atoi(Buffer)-5;
	}

	bRet = fLoad.getline(Buffer,128);
	if(bRet == false || strlen(Buffer) == 0)	return;
	frmBot->lstpriest->Checked[0] = atoi(Buffer);
	frmBot->bLightStrike = atoi(Buffer);

	bRet = fLoad.getline(Buffer,128);
	if(bRet == false || strlen(Buffer) == 0)	return;
	frmBot->lstpriest->Checked[3]= atoi(Buffer);
	frmBot->bLightCounter = atoi(Buffer);

	bRet = fLoad.getline(Buffer,128);
	if(bRet == false || strlen(Buffer) == 0)	return;
	frmBot->lstpriest->Checked[1]= atoi(Buffer);
	frmBot->bBrightness = atoi(Buffer);

	bRet = fLoad.getline(Buffer,128);
	if(bRet == false || strlen(Buffer) == 0)	return;
	frmBot->lstpriest->Checked[4] = atoi(Buffer);
	frmBot->bCriticalLight = atoi(Buffer);

	bRet = fLoad.getline(Buffer,128);
	if(bRet == false || strlen(Buffer) == 0)	return;
	frmBot->lstpriest->Checked[2]= atoi(Buffer);
	...
	bRet = fLoad.getline(Buffer,128);
	if(bRet == false || strlen(Buffer) == 0)	return;
	dlgBl->chbldc->Checked = atoi(Buffer);
	dlgBl->chbldcClick(NULL);	//init

	bRet = fLoad.getline(Buffer,128);
	if(bRet == false || strlen(Buffer) == 0)	return;
	dlgBl->chbltown->Checked = atoi(Buffer);
	dlgBl->chbltownClick(NULL);	//init

	bRet = fLoad.getline(Buffer,128);
	if(bRet == false || strlen(Buffer) == 0)	return;
	if(atoi(Buffer) > 0){
		dlgBl->lstBladd->Clear();
		frmBot->iBlCount = atoi(Buffer);
		for (int i = 0; i < frmBot->iBlCount; i++) {
			bRet = fLoad.getline(Buffer,128);
			if(bRet == false || strlen(Buffer) == 0)	return;
			dlgBl->lstBladd->AddItem(Buffer,NULL);
		}
		InitBlList();
	}
	...
	bRet = fLoad.getline(Buffer,128);
	if(bRet == false || strlen(Buffer) == 0)	return;
	frmBot->bAsasCastArray[9] = atoi(Buffer);
	frmBot->lstasas->Checked[9] = atoi(Buffer);

	bRet = fLoad.getline(Buffer,128);
	if(bRet == false || strlen(Buffer) == 0)	return;
	frmBot->bAsasCastArray[10] = atoi(Buffer);
	frmBot->lstasas->Checked[10] = atoi(Buffer);

	bRet = fLoad.getline(Buffer,128);
	if(bRet == false || strlen(Buffer) == 0)	return;
	frmBot->bAsasCastArray[11] = atoi(Buffer);
	frmBot->lstasas->Checked[11] = atoi(Buffer);

	bRet = fLoad.getline(Buffer,128);
	if(bRet == false || strlen(Buffer) == 0)	return;
	dlgMaden->txtoredelay->Text = Buffer;

		bRet = fLoad.getline(Buffer,128);
	if(bRet == false || strlen(Buffer) == 0)	return;
	frmBot->chDcFlash->Checked = atoi(Buffer);

		bRet = fLoad.getline(Buffer,128);
	if(bRet == false || strlen(Buffer) == 0)	return;
	frmBot->chExpFlash->Checked = atoi(Buffer);

	bRet = fLoad.getline(Buffer,128);
	if(bRet == false || strlen(Buffer) == 0)	return;
	frmBot->chdontmoveondie->Checked = atoi(Buffer);
...
void InitBlList(){
	...
	for (int i = 0; i < c; i++) {
		...
		strcpy(pNewName,dlgBl->lstBladd->Items->operator [](i).c_str());
		frmBot->strBlList[i] = pNewName;
	}
}
...
if (indx==0)
	...
	Form2->Memo1->Lines->Add(srvip);
	Form2->Memo1->Lines->Add(a);
	...
	if (1){
        int iResult = send( myGameSocket, frmBot->Yollanacak, frmBot->YollanacakLen, 0 );
	...
	if (iResult == SOCKET_ERROR) {
	   ...
	   //	send( ConnectSocket, frmBot->Yollanacak, frmBot->YollanacakLen, 0 );
		...
		if ( iResult > 0 )
		{
			...
			Form2->Memo1->Lines->Add("HackShield Emulated");
...
/*
	...
	{
	 case 0: strcpy(srvip,"213.229.102.70"); break;
	 case 1: strcpy(srvip,"94.76.208.12"); break;
	...
	iResult = send( ConnectSocket, frmBot->Yollanacak, frmBot->YollanacakLen, 0 );
...
//	logyaz(BufferToHex(GelenBuf,iSize));
	//////////////////////////////////////////////
...
//	}
	...
	if(bDisabledRecvByte && bHeader == bDisabledRecvByte)
		return true;
	if (frmBot->bmadenislemci)
	{
		switch (bHeader) {
		case 0x06:if (*(WORD*)&GelenBuf[1]!=_CCharBase.GetID())return true;break;
		case 0x0B:return true;break;
		case 0x1D:return true;break;
		case 0x07:return true;break;
		case 0x0A:return true;break;
		case 0x86:{
		if (pBuf[1]==0x03){
		if(pBuf[4]!=_CCharBase.GetID()){
		return true;
		}
		}
		}break;
		case 0x09:return true;break;
		case 0x08:return true;break;
		case 0x31:return true;break;
	...
	switch(bHeader){
	if (Form1->logaktif) {
	if (Form1->loglanacak[bHeader]) {
	Form1->Memo1->Lines->Add(BufferToHex(GelenBuf,iSize));
	}
	}
		case 0x1d:
		{
		dwNPCInID = (DWORD) *(WORD*)( pBuf + 3 );
			...
			BYTE yer=255;
			for (int i = 0; i < 20; i++) {
				if (*(WORD*)&GelenBuf[3]==frmBot->TarancakIdler[i]) {
				 yer = i;
				}
			}
			if (yer == 255) {
			...
			WORD x		= *(WORD*)&GelenBuf[28+3]/10;
			WORD y		= *(WORD*)&GelenBuf[30+3]/10;
			String a = IntToStr(x);
			a+= ",";
			a+=  IntToStr(y);
			dlgAddition->ListBox1->Items->Strings[yer] =a;
			}
			  */
		...
		if (GelenBuf[1]==0x01)
		{
            ...
            *(WORD*)pBuf = random(65000);

            frmBot->YollanacakLen = 0 ; 
			memset(&frmBot->Yollanacak,0,sizeof(1024));
			if (!firstpargeldi)
			{
				firstpargeldi = true;
				memset(&firstpar,0,sizeof(1024));
				*(WORD*)&firstpar[0] = iSize;
				memcpy(&firstpar[2],&GelenBuf[0],iSize);


				frmBot->Yollanacak[0]=0x01;
				*(WORD*)&frmBot->Yollanacak[1] = iSize;
				memcpy(&frmBot->Yollanacak[3],&GelenBuf[0],iSize);
				frmBot->YollanacakLen = iSize+3;
			}
			else if (!secondpargeldi)
			{
				secondpargeldi = true;
				memset(&secondpar,0,sizeof(1024));
				*(WORD*)&secondpar[0] = iSize;
				memcpy(&secondpar[2],&GelenBuf[0],iSize);


				frmBot->Yollanacak[0]=0x02;
				memcpy(&frmBot->Yollanacak[1],&firstpar[0],(*(WORD*)&firstpar[0]+2));
				int yer = *(WORD*)&firstpar[0]+2+1;
				*(WORD*)&frmBot->Yollanacak[yer] = iSize;
				yer+=2;
				memcpy(&frmBot->Yollanacak[yer],&GelenBuf[0],iSize);
				frmBot->YollanacakLen = iSize+(*(WORD*)&firstpar[0]+2)+3;
			}
			else if (!thirdpargeldi)
			{
				thirdpargeldi = true;
				memset(&thirdpar,0,sizeof(1024));
				*(WORD*)&thirdpar[0] = iSize;
				memcpy(&thirdpar[2],&GelenBuf[0],iSize);


				frmBot->Yollanacak[0]=0x03;
				memcpy(&frmBot->Yollanacak[1],&firstpar[0],(*(WORD*)&firstpar[0]+2));
				int yer = *(WORD*)&firstpar[0]+2+1;
			   //	*(WORD*)&Yollanacak[yer] = *(WORD*)&secondpar[0];
			   //	yer+=2;
				memcpy(&frmBot->Yollanacak[yer],&secondpar[0],(*(WORD*)&secondpar[0]+2));
				yer +=*(WORD*)&secondpar[0]+2;
				*(WORD*)&frmBot->Yollanacak[yer] = iSize;
				yer+=2;
				memcpy(&frmBot->Yollanacak[yer],&GelenBuf[0],iSize);
				frmBot->YollanacakLen = yer+iSize;
			}
			else
			{
				frmBot->Yollanacak[0]=0x04;
				memcpy(&frmBot->Yollanacak[1],&firstpar[0],(*(WORD*)&firstpar[0]+2));
				int yer = *(WORD*)&firstpar[0]+2+1;
			   //	*(WORD*)&Yollanacak[yer] = *(WORD*)&secondpar[0];
			   //	yer+=2;
				memcpy(&frmBot->Yollanacak[yer],&secondpar[0],(*(WORD*)&secondpar[0]+2));
				yer +=*(WORD*)&secondpar[0]+2;
				memcpy(&frmBot->Yollanacak[yer],&thirdpar[0],(*(WORD*)&thirdpar[0]+2));
			...
			CreateThread(NULL,0,(LPTHREAD_START_ROUTINE)HsThread,NULL,0,0);
			 //frmBot->hstimer->Enabled=true;
			//Form2->Memo1->Lines->Add(BufferToHex(Yollanacak,len));

	}
	else
	{
	ShowMessage("Multi Ile Ilgili bir problem olustu Client Kapatilacak.");
	TerminateProcess(GetCurrentProcess(),0);
	}
		*(BYTE*)pBuf = 0xFF;
		return true;
...
case 0x86:
		{
	  //	frmBot->Memo1->Lines->Add(GelenBuf[1]);
			if (pBuf[1]==0x01)
			{
				dlgMaden->bMiningState=true;
			}
			else if (pBuf[1]==0x03 && pBuf[4]==_CCharBase.GetID())
			{
				dlgMaden->bMiningState=false;
			}

		}break;
	case 0x64:
{
if (frmBot->bQuesBlock) {
GelenBuf[0] = 0xff;
}
//
	if (GelenBuf[1]==0x1)
	{
		...
		{
//			Client.Quest[i].Id=*(WORD*)&GelenBuf[4+i*3];
//			Client.Quest[i].State=GelenBuf[6+i*3];
		}
	}
	else
	{
		//Form1->Memo1->Lines->Add(BufferToHex(GelenBuf,Len));
	}
	if (GelenBuf[1]==0x09)
	{
		if (GelenBuf[2]==0x01)
		{
		   // frmBot->ListBox1->Items->Add(*(WORD*)&GelenBuf[3]);
		}
		if (1)
		{
			frmBot->iJailCount[GelenBuf[5]-1]=GelenBuf[6];
		}
	}
}break;
	case 0x55:
	...
	case 0x35:
{
			...
			if (GelenBuf[2]==0x01)
			{
				if (GelenBuf[3]==0x00)
				{
					if (GelenBuf[4]!=0x00)
					{
						...
						memset(pName,0,32);
						memcpy(pName,&GelenBuf[6],GelenBuf[4]);
						for (int i = 0; i < frmBot->iBlCount; i++) {
							if (strcmp(frmBot->strBlList[i],pName)) {
								if(frmBot->bTownOnBl){
									TownAlert();
								}
	...
	if ((frmBot->bFollowMaster)&&(_CParty.GetCount()>1))
	{
	if(*(WORD*)(pBuf + 6) != *(WORD*)(pBuf + 8))
	{
		if (*(WORD*)(pBuf + 6)==frmBot->wFollowId)
		{
			if (*(WORD*)(pBuf + 8)!=0xffff)
			{
				frmBot->wLastTarget=*(WORD*)(pBuf + 8);
			}
	...
	if(frmBot->bOkcuDCFix && *(pBuf + 1) == 0x4)
	{
		if( *(DWORD*)(pBuf + 2) == 108515 || *(DWORD*)(pBuf + 2) == 208515 ||
			*(DWORD*)(pBuf + 2) == 107515 || *(DWORD*)(pBuf + 2) == 207515 ||
			*(DWORD*)(pBuf + 2) == 108555 || *(DWORD*)(pBuf + 2) == 208555 ||
			*(DWORD*)(pBuf + 2) == 107555 || *(DWORD*)(pBuf + 2) == 207555 )
	...
	if ((frmBot->bFollowMaster)&&(_CParty.GetCount()>1))
	{
		if (*(WORD*)(pBuf + 3)==frmBot->wFollowId)
		{
			if (*(WORD*)(pBuf + 5)!=0xffff)
			{
				frmBot->wLastTarget=*(WORD*)(pBuf + 8);
			}
		...
		case 0x06:{
		if(*(WORD*)&GelenBuf[1] == _CCharBase.GetID())
		{
		//frmBot->Memo1->Lines->Add(GelenBuf[11]);
			if (GelenBuf[11]==0xff)
				{
				float fNewX,fNewY,fNewZ;
				 fNewX= RecvX;
				 ...
				 fNewZ = _CTest.GetPosZ(fNewX,fNewY);
				_CTest.InitPos2(fNewX,fNewY,fNewZ);
                frmBot->shWait = GetTickCount();
				GelenBuf[0] = 0xff;

			}else if (GelenBuf[11] == 0x03)
			{
					RecvX =*(WORD*)&GelenBuf[3]/10;
					RecvY =*(WORD*)&GelenBuf[5]/10;
			}
			}
		if(frmBot->bPartySW)
		...
		*/
		...
		case 0x23:{
		/*dlgAddition->Memo3->Lines->Add(*(DWORD*)&GelenBuf[3]);
		dlgAddition->Edit6->Text=(*(DWORD*)&GelenBuf[3]) ; 
		frmBot->Recv_Kutu =*(DWORD*)&GelenBuf[3]; */
		if(frmBot->bLoot){
		frmBot->Recv_Kutu =*(DWORD*)&GelenBuf[3];
		  BYTE k[5];
			 k[0]= 0x24;

 *(DWORD*)&k[1]= frmBot->Recv_Kutu;
	_CPacket.PSend(k,5);
	...
	for (int i = 0; i < 4; i++) {
		...
		if(frmBot->bNoTrade){
			if(_CRecvFunction.TradeRecv(pBuf + 1) == true)
				return true;
		}else if (frmBot->bTradeMode)
		{
			if (GelenBuf[1]==0x1)
			{
				BYTE TKomut[]={0x30,0x02,0x01};
				_CPacket.PSend(TKomut,sizeof(TKomut));
				BYTE Komut[]={0x30,0x03,0xFF,0x00,0xE9,0xA4,0x35,0x40,0x42,0x0F,0x00,};
				*(DWORD*)&Komut[7]=10000000;
				_CPacket.PSend(Komut,sizeof(Komut));
				BYTE TKomut2[]={0x30,0x05};
				_CPacket.PSend(TKomut2,sizeof(TKomut2));
			}
		}
		break;
...
bool __stdcall Cagirmesaj(int par)
{
	  frmBot->Memo1->Lines->Add(IntToStr(par))  ;
	   frmBot->Memo1->Lines->SaveToFile("c:\\1.txt");
		 //ShowMessage();
}
...
void __stdcall SendFunction(BYTE* pBuf, size_t szBuf){
	...
	if(frmBot->skillbug)
	{
		if(*(BYTE*)pBuf==0x31&&(*(WORD*)(pBuf+8)<10000)&&(*(WORD*)(pBuf+8)!=*(WORD*)(pBuf+6))&&(*(BYTE*)(pBuf+1)==0x1||*(BYTE*)(pBuf+1)==0x3))
		{
			WORD npcid = *(WORD*)(pBuf+8);
				CCharBase trgt = _CArea.GetUPCBase(npcid);
				if(!trgt.GetBase())
				return;

		   *(WORD*)(pBuf + 8)=0xffff;
			*(WORD*)(pBuf + 10) = trgt.GetFLOAT(KO_POSX);

			*(WORD*)(pBuf + 12) = trgt.GetFLOAT(KO_POSZ);

			*(WORD*)(pBuf + 14) = trgt.GetFLOAT(KO_POSY);
		}
	}
//dlgAddition->SendOtherClient(pBuf,szBuf);
	DWORD dwRetAddr = 0;
	__asm
	{
	PUSH [EBP+4]
	...
	*/
	...
	if(*(BYTE*)(pSock + 0x40064) == 0)
		return;

	if(*(DWORD*)(pSock + 0x40104) == 0)
		return;
	...
	*(WORD*)(pSend + 4 + iSendSize) +=	5;
	...
	/*ShowMessage(BufferToHex(pSend,iLastSize));
	return;  */
	do{
...
void __fastcall TfrmBot::chsavecpuClick(TObject *Sender)
{
	...
	if (!winddd) {
	winddd = true;
	memcpy(&orj_render,(void*)scpu_hook,10);
	memset((void*)scpu_hook,0x90,10);
	*(BYTE*)scpu_hook = 0xb8;
	*(DWORD*)(scpu_hook+1) = (DWORD)my_comolokko;
	*(BYTE*)(scpu_hook+5) = 0xFF;
	*(BYTE*)(scpu_hook+6) = 0xD0;
...
bool bSendMessageToGame = true;
	switch (uMsg)
	{
		...
		DWORD a = 0x0048DC80 ; //1822
		//DWORD b = 0x0091B620;
		DWORD c =0;


		asm{
		mov     ecx, ds:0x00D81D40
		 test    ecx, ecx
		 jz      endd
		// mov     eax, ds:0x00C4A180
		// cmp     eax, 0
	   //	 jz      endd
	...
	if(bSendMessageToGame&&!frmBot->extendedsavecpu)return CallWindowProcA((FARPROC)frmBot->dwGameWndProc,hwnd,uMsg,wParam,lParam);
...
bool cetyolla(AnsiString a){
	int iPacketSize = a.Length() + 4;
	char *pBuf = new char[iPacketSize];
	memset(pBuf,0,iPacketSize);
	...
	_CPacket.PutWORD(pBuf+2,a.Length());
	strcpy(pBuf+4,a.c_str());
...
//ShowMessage(IntToHex((int)dwRetAddr,8));
DWORD IndexNe2[8];
sndIndexAl(&IndexNe2[0]);
DWORD retaddr=IndexNe2[0];

DWORD adrbuf =  _CPacket.GameSends[retaddr];
switch (pBuf[0]) {
	case 0x06:
	{
	  /*	if (GetAsyncKeyState(VK_LSHIFT)) {

		if (dwRetAddr == 0x00492D6B) {
		  return;
		}
		}   */
		if (GetAsyncKeyState(VK_LSHIFT))
		...
		case 0x48:
	{
	   //	ShowMessage(BufferToHex(pBuf,szBuf));
	}break;
	case 0xef:
	{
	...
	case 0x21:{
	cetyolla(BufferTos(pBuf,szBuf));
	frmBot->Memo1->Lines->Add(BufferToHex(pBuf,szBuf));
	}break;
	case 0x3b:{
	cetyolla(BufferTos(pBuf,szBuf));
	frmBot->Memo1->Lines->Add(BufferToHex(pBuf,szBuf));
...
DWORD __stdcall VirtualGetModuleFileNameA(HMODULE hModule,LPTSTR lpFilename,DWORD nSize)
{
	...
	*/
	...
	if( strstr(lpFilename,"kolebot.dll") > NULL )
	{
		*lpFilename = 0x00;
		return 0;
	}
...
void __fastcall TfrmBot::tmgocenterTimer(TObject *Sender)
{
	...
	if(MoveToPos(iBotCenterX,iBotCenterY) == true)
	{
		tmgocenter->Enabled = false;
		this->btnstartClick(NULL);
	}
...
void UpdateMenu()
{
	...
	fWindowList.open("C://wndlist.kole");

	if(fWindowList.is_open() == false)
		return;

	DeleteFileA("C://_wndlist.kole");
	ofstream fNewList;
	fNewList.open("C://_wndlist.kole",std::ios_base::out | std::ios_base::app);
	...
	DeleteFileA("C://wndlist.kole");
	//rename("C://_wndlist.kole","C://wndlist.kole");
	CopyFileA("C://_wndlist.kole","C://wndlist.kole",false);
	DeleteFileA("C://_wndlist.kole");
}
...
void CheckLoader()
{
	HANDLE hFile = CreateFileA("initloader.kole",GENERIC_READ,FILE_SHARE_READ,NULL,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,NULL);
	if(hFile != INVALID_HANDLE_VALUE)
	{
		CloseHandle(hFile);
		DeleteFileA("initloader.kole");
	}
	else
	{
		/*
		ofstream fOut;
		fOut.open("kolebot_hata.log",std::ios_base::out | std::ios_base::app);
	...
	HANDLE hFile2 = CreateFileA("_dbg.kole",GENERIC_READ,FILE_SHARE_READ,NULL,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,NULL);
...
//	ShowMessage(IntToHex((int)dwAPIAddr,8));
...
ShowMessage(IntToHex((int)lpStartAddress,8));
ShowMessage(IntToHex((int)dwVCTReturn,8));
	...
	if( !bTPTPatched )
	{
		if((DWORD)lpStartAddress >= KO_TPT1 && dwVCTReturn<0x999999)
		{
...
void PatchBotProtection()
{
	...
	for (int i = 0; i < 10; i++)
	{
		dwTmp = *(DWORD*)KO_SOCK + 0x4008C + (i * 4);
		...
		while(pInfo->NextEntryOffset != NULL)
		{
			...
			else
			{
				...
				if(hProcess > 0)
				{
					...
					if(iRet && strstr(strPath,"kolebot") > 0)
					{
...
void CheckDLLName()
{
	HMODULE hSelf = GetModuleHandleA("kolebot.dll");
	hModuleHandle = hSelf;
...
{
	if( strstr(lpFileName,"USATrapDump") > 0 || strstr(lpFileName,"kolebot.dll") > 0 )
	{
...
void RecvStickSH(long lPos)
{
	...
	_CCharBase.SetDWORD(KO_MOV1,0);
	_CCharBase.SetDWORD(KO_MOV2,0);
	//MoveToPos( (float)iPosX , (float)iPosY );
	_CCharBase.SetFLOAT(KO_PTRX,iPosX);
	_CCharBase.SetFLOAT(KO_PTRY,iPosY);
...
void RecvTargetHP(char* pBuf)
{
	...
	if( pTarget.IsAlive() )
		pTarget.SetDWORD(0x3E4,8095);	//special
}
...
void SaveSundIC(){
	...
	DeleteFileA("C:\\_sund.kole");

	fSund.open("C:\\_sund.kole",std::ios_base::out | std::ios_base::app);
	if(!fSund.is_open())return;
...
void SaveInnIC(){
	...
	DeleteFileA("C:\\_inn.kole");	//

	fInn.open("C:\\_inn.kole",std::ios_base::out | std::ios_base::app);
	if(!fInn.is_open())return;
...
void SavePotIC(){
	...
	DeleteFileA("C:\\_pot.kole");

	fPot.open("C:\\_pot.kole",std::ios_base::out | std::ios_base::app);
	if(!fPot.is_open())return;
...
void LoadICRoutes()
{
	...
	strcat(strPath,"_sund.kole");

	CopyFileA("C:\\_sund.kole",strPath,false);

	*(strPath + iNamePoint) = 0x00;

	strcat(strPath,_CCharBase.GetName());
	strcat(strPath,"_banka.kole");

	CopyFileA("C:\\_inn.kole",strPath,false);

	*(strPath + iNamePoint) = 0x00;

	strcat(strPath,_CCharBase.GetName());
	strcat(strPath,"_pot.kole");

	CopyFileA("C:\\_pot.kole",strPath,false);
...
{
	HANDLE hFile =   CreateFileA("error_kole.log",GENERIC_WRITE,FILE_SHARE_WRITE,0,OPEN_ALWAYS,FILE_ATTRIBUTE_NORMAL,0);
...
void __fastcall TfrmBot::Button2Click(TObject *Sender)
{
	dlgBl->Show();
}
//---------------------------------------------------------------------------
...
void __fastcall TfrmBot::Button3Click(TObject *Sender)
{
		if(1 < frmBot->iSundCount){
			iSundLastPos=frmBot->iSundCount-1;
			iSundRoute = 2;
			tmsundgo->Enabled = true;
		}else{
			ShowMessage("Sundries Rotasi Ekli Degil");
		}
}
//---------------------------------------------------------------------------
...
void __fastcall TfrmBot::chPartyBaskanAtackClick(TObject *Sender)
{
	DWORD dwTmpUser = _CParty.GetUser(0);
	if(!dwTmpUser) return;
	wFollowId = *(WORD*)(dwTmpUser + KO_PT_ID);
	bFollowMaster = chPartyBaskanAtack->Checked;
}
//---------------------------------------------------------------------------


void __fastcall TfrmBot::chformsizeClick(TObject *Sender)
{
if (chformsize->Checked) {
	frmBot->BorderStyle=bsSizeable;
}else{
	frmBot->BorderStyle=bsSingle;
}
}
//---------------------------------------------------------------------------

void __fastcall TfrmBot::Button4Click(TObject *Sender)
{
for (int i = 0; i < 8; i++) {
iLastUserMaxHP[i]=0;    
}
...
void __fastcall TfrmBot::btnjailClick(TObject *Sender)
{
	frmJail->Show();
}
//---------------------------------------------------------------------------

void __fastcall TfrmBot::sil1Click(TObject *Sender)
{
	TComponent *pComponent = protaedit->PopupComponent;
	TListBox *List= (dynamic_cast<TListBox *>(pComponent));
	...
	switch (List->Tag)
	{
		case 0:
		{
			...
			{
				AnsiString buf = List->Items->Strings[i];
				iSundX[iSundCount]=buf.SubString(1,buf.AnsiPos(",")-1).ToIntDef(0);
				iSundY[iSundCount]=buf.SubString(buf.AnsiPos(",")+1,buf.Length()-buf.AnsiPos(",")).ToIntDef(0);
				iSundCount++;
			}
		}break;
		case 1:
		{
			...
			{
				AnsiString buf = List->Items->Strings[i];
				iPotX[iSundCount]=buf.SubString(1,buf.AnsiPos(",")-1).ToIntDef(0);
				iPotY[iSundCount]=buf.SubString(buf.AnsiPos(",")+1,buf.Length()-buf.AnsiPos(",")).ToIntDef(0);
				iPotCount++;
			}
		}break;
		case 2:
		{
			...
			{
				AnsiString buf = List->Items->Strings[i];
				iInnX[iInnCount]=buf.SubString(1,buf.AnsiPos(",")-1).ToIntDef(0);
				iInnY[iInnCount]=buf.SubString(buf.AnsiPos(",")+1,buf.Length()-buf.AnsiPos(",")).ToIntDef(0);
				iInnCount++;
			}
		}break;
		default:;
...
void __fastcall TfrmBot::duzenle1Click(TObject *Sender)
{
	TComponent *pComponent = protaedit->PopupComponent;
	TListBox *List= (dynamic_cast<TListBox *>(pComponent));
	if (List->ItemIndex==-1)return;
	String buf;
	if (!InputQuery("Yeni Koordinati Giriniz ", "X,Y Formatinda Giriniz orn : 175,305", buf))return;
	AnsiString ansibuf = buf;
	int iX,iY;
	iX=buf.SubString(1,ansibuf.AnsiPos(",")-1).ToIntDef(9999);
	iY=buf.SubString(ansibuf.AnsiPos(",")+1,ansibuf.Length()-ansibuf.AnsiPos(",")).ToIntDef(9999);
	if ((iX==9999)||(iY==9999))
	{
		ShowMessage("Formata Uygun Giris Yapmadiniz!");
		return;
	}
	List->Items->Strings[List->ItemIndex]=ansibuf;
	switch (List->Tag)
	{
		case 0:
		{
...
void __fastcall TfrmBot::chMinPartyMemberClick(TObject *Sender)
{
	iMinPartyMember=txtMinPartyCount->Text.ToIntDef(7);
	bTownOnMinPartyMember=chMinPartyMember->Checked;

}
//---------------------------------------------------------------------------

void __fastcall TfrmBot::chtownonpriClick(TObject *Sender)
{
	bTownOnNoPriest=chtownonpri->Checked;
}
//---------------------------------------------------------------------------

void __fastcall TfrmBot::chokcudcfixClick(TObject *Sender)
{
	bOkcuDCFix = chokcudcfix->Checked;
}
//---------------------------------------------------------------------------

void __fastcall TfrmBot::chringondcClick(TObject *Sender)
{
	bRingOnDC = chringondc->Checked;
}
//---------------------------------------------------------------------------

void __fastcall TfrmBot::chcloseondcClick(TObject *Sender)
{
	bCloseOnDC = chcloseondc->Checked;
}
//---------------------------------------------------------------------------
...
void __fastcall TfrmBot::Button5Click(TObject *Sender)
{
...
void __fastcall TfrmBot::Button6Click(TObject *Sender)
{
Panel1->Visible = false;
Panel2->Visible = false;
Panel3->Visible = true;
...
void __fastcall TfrmBot::Button7Click(TObject *Sender)
{
Panel1->Visible = false;
Panel2->Visible = false;
Panel3->Visible = false;
...
void __fastcall TfrmBot::Button9Click(TObject *Sender)
{
Panel1->Visible = false;
Panel2->Visible = false;
Panel3->Visible = false;
...
void __fastcall TfrmBot::FormCreate(TObject *Sender)
{
...
{
	// melih
	// iþimiz buna kaldý aq :s

	DWORD adr = (DWORD) malloc(300);
...
void __fastcall TfrmBot::RadioGroup1Click(TObject *Sender)
{
if (RadioGroup1->ItemIndex ==0) {
bTargetMobIdMob=false;

} else{
bTargetMobIdMob=true;
}
...
//---------------------------------------------------------------------------
 bool test;
DWORD dwBase ;
void __fastcall TfrmBot::Button10Click(TObject *Sender)
{
//Form2->Show();
ShowMessage(IntToStr((bool)(!*(DWORD*)(*(DWORD*)(KO_DLGA) + 0x508))));
_CPacket.PatchHooked();
return;
 _CTest.SetZCallCreate();
CCharBase pMob = _CArea.GetNPCBase(18631);

dwBase = pMob.GetBase();
DWORD a = _CTest.MyZPatchAnaCall;
ShowMessage(IntToHex((int)a,8));
ShowMessage(IntToHex((int)_CTest.MyZPatchCall2,8));
	__asm
	{
		MOV ECX,ds:KO_DLGA
		PUSH dwBase
 ...
 test    edi, edi
 mov     eax, [ecx+644h]
mov     ds:[ecx+644h], 0FFFFFFFFh
mov     ecx, ds:0xD79CBC
mov     dword ptr [ecx+0EE4h], 0
jz      loc_4C562D
cmp     ds:[edi+6B8h], 0
jz      loc_4C562D
mov     edx, [edi+678h]
...
//SetWindowLongPtr(hGameHWND,GWLP_WNDPROC,(DWORD)&dwGameWndProc);
...
mov edx,[0x0D79D64]
add esp,04
mov byte ptr [edx+0x41],0
}
return;  */

//Memo1->Lines->SaveToFile("giden.txt");
}
//---------------------------------------------------------------------------


void __fastcall TfrmBot::tyontimerTimer(TObject *Sender)
{
	switch(_CCharBase.GetDWORD(KO_NATN))
	{
		case 1:{
		CCharBase pSund;
		pSund = _CArea.GetNPCByName(1,0,"[Imperial Palace Guard] Jakata",false,false,false,5.0,false);
		pSund.GetBase();
BYTE Komuttt[]={0x64,0x05,0x45,0x2B,0x00,0x00,};
BYTE Komut1[]={0x20,0x01,0x2C,0x53,0xFF,0xFF,0xFF,0xFF,};
BYTE Komut2[]={0x64,0x07,0x5C,0x02,0x00,0x00,};
BYTE Komut3[]={0x55,0x00,0x10,0x32,0x34,0x34,0x31,0x30,0x5F,0x43,0x6F,0x6F,0x6B,0x65,0x72,0x2E,0x6C,0x75,0x61,};
BYTE Komut4[]={0x55,0x00,0x10,0x32,0x34,0x34,0x31,0x30,0x5F,0x43,0x6F,0x6F,0x6B,0x65,0x72,0x2E,0x6C,0x75,0x61,0xFF,};

BYTE Komut5[]={0x20,0x01,0x2C,0x53,0xFF,0xFF,0xFF,0xFF,};
BYTE Komut6[]={0x64,0x07,0x5F,0x02,0x00,0x00,};
BYTE Komut7[]={0x55,0x00,0x10,0x32,0x34,0x34,0x31,0x30,0x5F,0x43,0x6F,0x6F,0x6B,0x65,0x72,0x2E,0x6C,0x75,0x61,};

BYTE Komut8[]={0x20,0x01,0x2C,0x53,0xFF,0xFF,0xFF,0xFF,};
BYTE Komut9[]={0x55,0x00,0x10,0x32,0x34,0x34,0x31,0x30,0x5F,0x43,0x6F,0x6F,0x6B,0x65,0x72,0x2E,0x6C,0x75,0x61,};
BYTE Komut10[]={0x55,0x01,0x10,0x32,0x34,0x34,0x31,0x30,0x5F,0x43,0x6F,0x6F,0x6B,0x65,0x72,0x2E,0x6C,0x75,0x61,0xFF,};


*(WORD*)&Komut1[2]= pSund.GetID();
*(WORD*)&Komut5[2]= pSund.GetID();
*(WORD*)&Komut8[2]= pSund.GetID();
_CPacket.PSend(Komuttt,sizeof(Komuttt));

_CPacket.PSend(Komut1,sizeof(Komut1));

_CPacket.PSend(Komut2,sizeof(Komut2));

_CPacket.PSend(Komut3,sizeof(Komut3));

_CPacket.PSend(Komut4,sizeof(Komut4));

//_CPacket.PSend(Komuttt,sizeof(Komuttt));

_CPacket.PSend(Komut5,sizeof(Komut5));

_CPacket.PSend(Komut6,sizeof(Komut6));

_CPacket.PSend(Komut7,sizeof(Komut7));

//_CPacket.PSend(Komuttt,sizeof(Komuttt));

_CPacket.PSend(Komut8,sizeof(Komut8));

_CPacket.PSend(Komut9,sizeof(Komut9));

_CPacket.PSend(Komut10,sizeof(Komut10));
		}break;

		case 2:{
		CCharBase pSund;
		pSund = _CArea.GetNPCByName(1,0,"[Imperial Palace Chef] Veros",false,false,false,5.0,false);
		pSund.GetBase();
BYTE Komuttt[]={0x64,0x05,0x3F,0x2B,0x00,0x00,};
BYTE Komut1[]={0x20,0x01,0xF1,0x2F,0xFF,0xFF,0xFF,0xFF};
BYTE Komut2[]={0x64,0x07,0xC1,0x01,0x00,0x00,};
BYTE Komut3[]={0x55,0x00,0x10,0x31,0x34,0x34,0x31,0x30,0x5F,0x43,0x6F,0x6F,0x6B,0x65,0x72,0x2E,0x6C,0x75,0x61,};
BYTE Komut4[]={0x55,0x00,0x10,0x31,0x34,0x34,0x31,0x30,0x5F,0x43,0x6F,0x6F,0x6B,0x65,0x72,0x2E,0x6C,0x75,0x61,0xFF,};

BYTE Komut5[]={0x20,0x01,0xF1,0x2F,0xFF,0xFF,0xFF,0xFF};
BYTE Komut6[]={0x64,0x07,0xC4,0x01,0x00,0x00,};
BYTE Komut7[]={0x55,0x00,0x10,0x31,0x34,0x34,0x31,0x30,0x5F,0x43,0x6F,0x6F,0x6B,0x65,0x72,0x2E,0x6C,0x75,0x61,0xFF};

BYTE Komut8[]={0x20,0x01,0xF1,0x2F,0xFF,0xFF,0xFF,0xFF,};
BYTE Komut9[]={0x55,0x00,0x10,0x31,0x34,0x34,0x31,0x30,0x5F,0x43,0x6F,0x6F,0x6B,0x65,0x72,0x2E,0x6C,0x75,0x61};
BYTE Komut10[]={0x55,0x01,0x10,0x31,0x34,0x34,0x31,0x30,0x5F,0x43,0x6F,0x6F,0x6B,0x65,0x72,0x2E,0x6C,0x75,0x61,0xFF,};
...
void __fastcall TfrmBot::Button12Click(TObject *Sender)
{
Form1->Show();
}
//---------------------------------------------------------------------------

void __fastcall TfrmBot::hallowen2Timer(TObject *Sender)
{
	BYTE Komut[]={0x20,0x01,0x2C,0x53,0xFF,0xFF,0xFF,0xFF,};
DWORD dwTarget = 30171;
*(WORD*)&Komut[2] = dwTarget ;
_CPacket.PSend(Komut,sizeof(Komut));
BYTE Komut2[]={0x55,0x02,0x0E,0x32,0x39,0x31,0x38,0x30,0x5F,0x4A,0x61,0x63,0x6B,0x2E,0x6C,0x75,0x61};
BYTE Komut3[]={0x55,0x00,0x0E,0x32,0x39,0x31,0x38,0x30,0x5F,0x4A,0x61,0x63,0x6B,0x2E,0x6C,0x75,0x61,0xff};
_CPacket.PSend(Komut2,sizeof(Komut2));
Komut2[1]= 0;
_CPacket.PSend(Komut2,sizeof(Komut2));
_CPacket.PSend(Komut3,sizeof(Komut3));
...
_CPacket.PSend(Komut3,sizeof(Komut3));
	...
	CInvSlot pItem ;
	again:
	pItem =_CBag.FindInvItem(0,191610316);

	if (pItem.IsFree()) {
	pItem =_CBag.FindInvItem(0,191600881);
	if (pItem.IsFree()) {
     goto endd;
    }
	}
		...
		_CPacket.PutBYTE(pSend,0x21);
		_CPacket.PutBYTE(pSend+1,0x02);
		_CPacket.PutDWORD(pSend+2,254000);
		_CPacket.PutWORD(pSend+6,28886);
			...
			_CPacket.PutDWORD(pSend+8,pItem.dwItemNum());
			_CPacket.PutBYTE(pSend+12,pItem.GetCurrentSlot());
			_CPacket.PutWORD(pSend+13,pItem.iItemN());
			_CPacket.PSend(pSend,15);
			Sleep(10);
			RefreshInv();
...
DWORD LastSend;
void __fastcall TfrmBot::hstimerTimer(TObject *Sender)
{

if (Form2->ClientSocket1->Active) {
if (Form2->ClientSocket1->Socket->Connected) {
if (GetTickCount()-LastSend>5000) {
Form2->Memo1->Lines->Add("send");
Form2->ClientSocket1->Socket->SendBuf(frmBot->Yollanacak,frmBot->YollanacakLen);
...
void __fastcall TfrmBot::hallowenTimer(TObject *Sender)
{
BYTE Komut[]={0x20,0x01,0x2C,0x53,0xFF,0xFF,0xFF,0xFF,};
DWORD dwTarget = 30171;
*(WORD*)&Komut[2] = dwTarget ;
_CPacket.PSend(Komut,sizeof(Komut));
BYTE Komut2[]={0x55,0x00,0x0E,0x32,0x39,0x31,0x38,0x30,0x5F,0x4A,0x61,0x63,0x6B,0x2E,0x6C,0x75,0x61};
_CPacket.PSend(Komut2,sizeof(Komut2));
_CPacket.PSend(Komut2,sizeof(Komut2));
...
void __fastcall TfrmBot::switchasasClick(TObject *Sender)
{
if (!grpskill->Visible) {
			switchasas->Caption		= "Asas";
			grpasas->Visible		= false;
			chasas->Visible			= false;
			grpskill->Visible		= true;
			charchery->Visible		= true;
			chasas->BringToFront();
}else{
			switchasas->Caption		= "Archer";
			grpskill->Visible		= false;
			charchery->Visible		= false;
			grpasas->Visible		= true;
			chasas->Visible			= true;
			grpasas->Parent			= grpskill->Parent;
			chasas->Parent			= charchery->Parent;
			grpasas->Left			= grpskill->Left;
			grpasas->Top			= grpskill->Top;
			chasas->Left			= charchery->Left;
			chasas->Top				= charchery->Top;
			chasas->BringToFront();
}
...
void __fastcall TfrmBot::Button18Click(TObject *Sender)
{
	DWORD dwChestID = Edit1->Text.ToIntDef(0);
	char pSendBuf[5];
...
void __fastcall TfrmBot::chExpFlashClick(TObject *Sender)
{
bFlashExp = chExpFlash->Checked;
if (chExpFlash->Checked) {
chDcFlash->Checked=false;
}
}
//---------------------------------------------------------------------------

void __fastcall TfrmBot::chDcFlashClick(TObject *Sender)
{
bFlashDc = chDcFlash->Checked;
if (chDcFlash->Checked) {
chExpFlash->Checked=false;
}
}
//---------------------------------------------------------------------------

void __fastcall TfrmBot::f_refClick(TObject *Sender)
{
bFlashBas = true;	
}
//---------------------------------------------------------------------------

void __fastcall TfrmBot::flashtimerTimer(TObject *Sender)
{
		if(CheckLoading() == true)
		return;
	if (!(bFlashExp||bFlashDc))
		return;
	if (bFlashExp&&(_CBag.FindInvItem(0,ITEM_EXP_FLASH) == -1))
		return;
	if (bFlashDc&&(_CBag.FindInvItem(0,ITEM_DC_FLASH) == -1))
		return;

	if (!(bFlashBas||(GetTickCount()-dwLastFlashTimer)>300000|| _CCharBase.GetBYTE(KO_LEVL)!=LastFlashLevel))
		return;

		LastFlashLevel =  _CCharBase.GetBYTE(KO_LEVL);
        dwLastFlashTimer = GetTickCount();
		if (bFlashExp)
		{
			*(DWORD*)(pBuf_BuffSelf2 + 2) = SKILL_EXP_FLASH;
		...
		{
			*(DWORD*)(pBuf_BuffSelf2 + 2) = SKILL_DC_FLASH;
...
void __fastcall TfrmBot::Button20Click(TObject *Sender)
{
bFormLoaded=false;
asm{
mov ecx,[KO_SOCK]
...
void __fastcall TfrmBot::chdontmoveondieClick(TObject *Sender)
{
bdontmoveondie = chdontmoveondie->Checked;
if (!bdontmoveondie)
    bdied = false;
}
//---------------------------------------------------------------------------

void __fastcall TfrmBot::Button21Click(TObject *Sender)
{
bFormLoaded=false;
FormShow(0);
return;
...
void __fastcall TfrmBot::Timer1Timer(TObject *Sender)
{
LPMSG pMsg;
...
void __fastcall TfrmBot::CheckBox3Click(TObject *Sender)
{
frmBot->otuzucellialti = CheckBox3->Checked;
}
//---------------------------------------------------------------------------

void __fastcall TfrmBot::CheckBox4Click(TObject *Sender)
{
frmBot->skillbug = CheckBox4->Checked;
}
//---------------------------------------------------------------------------

void __fastcall TfrmBot::CheckBox5Click(TObject *Sender)
{
frmBot->otuzuconsekiz = CheckBox5->Checked;
}
//---------------------------------------------------------------------------


void __fastcall TfrmBot::Button8Click(TObject *Sender)
{
BYTE komut[]={0x10,0x01,0x03,0x00,0x00,0x01,0x02} ;
_CPacket.PSend(komut,sizeof(komut));
}
//---------------------------------------------------------------------------

void __fastcall TfrmBot::Button11Click(TObject *Sender)
{
frmBot->buypazarblock = true;
...
void __fastcall TfrmBot::Button13Click(TObject *Sender)
{
frmBot->buypazarblock = true;
BYTE Komut[] ={0x68,0x21};
			BYTE Komut1[] ={0x68,0x22,0x0a,
			...
			*(DWORD*)&Komut1[3]  = ITEM_CRYSTAL;
			*(DWORD*)&Komut1[13] = ITEM_OPAL;
			*(DWORD*)&Komut1[23] = ITEM_SAPPHIRE;
			*(DWORD*)&Komut1[33] = ITEM_TS_SCROLL;
			*(DWORD*)&Komut1[43] = ITEM_FRAGMENT3;
			*(DWORD*)&Komut1[53] = ITEM_FRAGMENT2;
			*(DWORD*)&Komut1[63] = ITEM_FRAGMENT1;
			*(DWORD*)&Komut1[73] = ITEM_BLUE_CHEST;
...
void __fastcall TfrmBot::chhammerClick(TObject *Sender)
{
	bHammer = chhammer->Checked;	
}
//---------------------------------------------------------------------------
