...
//	c->Free();
	...
	*(pFileName + 1) = '\0';
	...
	int iMemSize = strlen("kolebot.dll") + 1;
	...
	*(pFileName + 1) = '\0';
	strcat(strPath,"initloader.kole");
	CreateFileA(strPath,GENERIC_READ,FILE_SHARE_READ,NULL,CREATE_ALWAYS,FILE_ATTRIBUTE_NORMAL,NULL);
	WriteProcessMemory(hProcess,lpVirtualMem2,"kolebot.dll",iMemSize - 1,NULL);
...
// ShowMessage(Calis);
 //ShowMessage(FileName);
 if (FindWindowA(NULL,"swixdorfswxswx")) {
 String NewExeName = DllPath + "\\..\\swx\\swixdorf.swx";
...
void LoadProcess(){
	...
	*(pFileName + 1) = '\0';
	...
	//*(pFileName + 1) = '\0';
	//strcat(strPath,"Launcher.exe");      //E03ED890-8E94-4B42-B1C5-3CDA401AA9C2
		...
		//CopyFile(frmLoader->txtPath->Text.c_str(),newexe,false);
	   //	AnsiString KoP;
	  //	   KoP.SetLength(255);
		//   strcpy(KoP.c_str(),AnsiString(AnsiString(strPathDir)+"\\").c_str());
	   //	KoP=AnsiString(strPathDir)+"\\";
		DllRunLancur(String(strPathDir),"kmodule.dat");


//	bool bProcess = CreateProcess(frmLoader->txtPath->Text.c_str()," E03ED890-8E94-4B42-B1C5-3CDA401AA9C2",NULL,NULL,true,NULL,NULL,strPathDir,&si,&pi);
...
//	FARPROC hLoadLibrary = GetProcAddress(hKernel32, "LoadLibraryA");
//
//	int iMemSize = strlen("kole.dll") + 1;
//
...
//	strcat(strPath,"initloader.kole");
//	CreateFileA(strPath,GENERIC_READ,FILE_SHARE_READ,NULL,CREATE_ALWAYS,FILE_ATTRIBUTE_NORMAL,NULL);
//	WriteProcessMemory(pi.hProcess,lpVirtualMem,"kole.dll",iMemSize - 1,NULL);
