...
char* strServerIP 	= 	"81.22.99.194";
char* strWebAddress	=	"http://onlinehile.org/kolebot/main.html";
...
//	__asm{AND dwMyID,0xFFFF};
	...
	*(WORD*)(pBuf_Attack4 + 6) = dwMyID;
	...
	if(iAttackType != 1){	//mage-warrior-priest
	  ...
	  ShowMessage(IntToHex((int)_CCharBase.GetBase(),8));
...
//	*(DWORD*)KO_APRB = 0;	// APR
  ...
  //	trayMain->BalloonHint = _CCharBase.GetName();         swixdorf
  //	trayMain->Hint = _CCharBase.GetName();

   //	if(bLoadSettings == true)
  //		LoadSettings();      swixdorf
...
//	}
  ...
  //	_CGeneral.PostMsgOut("### kole test [1917] ###",0xFFFFFFFF);	//test lan
...
void __fastcall TfrmBot::tmsundgoTimer(TObject *Sender)
{
	...
	if(iSundRoute == 1){	//gidiþ xd
		if(bRouteSH){
			if(SHTeleport(frmBot->iSundX[iSundLastPos],frmBot->iSundY[iSundLastPos],2))
				iSundLastPos++;
		}else{
...
void __fastcall TfrmBot::tmpotgoTimer(TObject *Sender)
{
	...
	if(iPotRoute == 1){	//gidiþ xd
		if(bRouteSH){
			if(SHTeleport(frmBot->iPotX[iPotLastPos],frmBot->iPotY[iPotLastPos],2))
				iPotLastPos++;
		}else{
...
void __fastcall TfrmBot::tminngoTimer(TObject *Sender)
{
	...
	if(iInnRoute == 1){	//gidiþ xd
		if(bRouteSH){
			if(SHTeleport(frmBot->iInnX[iInnLastPos],frmBot->iInnY[iInnLastPos],2))
				iInnLastPos++;
		}else{
...
//---------------------------------------------------------------------------
bool __stdcall ProcessRecv1(DWORD DataPack,int &iOffset)
{
ProcessRecv (DataPack,iOffset);
...
bool __stdcall ProcessRecva(DWORD DataPack,DWORD retaddra,int &iOffset)
{
DWORD IndexNe[8];
...
//	DWORD dwCallAddr = (_CRecv.GetIndex() * 4) + *(DWORD*)KO_PRTY + 0x18;

//	DWORD dwProcessAddr = *(DWORD*)dwCallAddr;

frmBot->Memo1->Lines->Add(IntToHex((int)*(DWORD*)retaddr,8));
frmBot->Memo1->Lines->Add(IntToHex(GelenBuf[0],8));
frmBot->Memo1->Lines->Add(IntToHex(iSize,8));
//frmBot->Memo1->Lines->Add(IntToHex((int)dwCallAddr,8));
...
//	break;
	...
	case 0x64:
{
//frmBot->ListBox1->Items->Add(*(WORD*)&GelenBuf[1]);
	if (GelenBuf[1]==0x1)
	{
...
//				jnz     loc_92E74B
//
//loc_92E6B9:                             //; CODE XREF: sub_92E630+7Ej
//
//                mov     eax, [esi+10h]
//                lea     ecx, [eax+4]
//                mov     [esi+10h], ecx
...
//				mov     ecx, ds:0xC49148
//                push    edi
//				call    dword ptr [ecx+edx*4+18h]
//                test    al, al
//                jnz     short loc_92E790
//                cmp     dword ptr [edi+4], 1
//                jge     short loc_92E76F
//				mov     byte ptr var_38, bl
//                jmp     short loc_92E778
//
//
//loc_92E76F:
//			   push 6
...
//                mov     eax, [edi+8]
//                mov     cl, [eax]
//				mov     byte ptr var_38, cl
//
//loc_92E778:
////				mov     edx, [esp+4Ch+var_38]
////				and     edx, 0FFh
////				push    edx
////				push    offset aInvalidPacket_
////				call    sub_4663A0
////				add     esp, 8
//
//loc_92E790:
...
//				jz      loc_92E8B7
//
//loc_92E7C6:                           //  ; CODE XREF: sub_92E630+281j
//                add     eax, 400C4h
//				mov     var_34, ebx
//                mov     ecx, eax
//                mov     edx, [ecx]
...
//                call    dword ptr [edx]
//
//loc_92E812:                           //  ; CODE XREF: sub_92E630+1D8j
//                mov     eax, ds:0xC4912C
//                mov     edx, [eax+400CCh]
//                mov     ecx, [eax+400ECh]
//                lea     esi, [eax+400C0h]
...
//                jz      short loc_92E83C
//                cmp     eax, [esi+8]
//                jnz     short loc_92E8A6
//
//loc_92E83C:                         //    ; CODE XREF: sub_92E630+205j
//				mov     eax, [esi+10h]
//                lea     ecx, [eax+4]
//                mov     [esi+10h], ecx
//                mov     edx, [eax]
...
//				add     esp, 4
//				pop ebp
//                jmp     short loc_92E8A6
//
//
//loc_92E88B:                           //  ; CODE XREF: sub_92E630+225j
//				mov     eax, [esi+10h]
//				add     esi, 4
//				mov     ecx, [eax]
//				mov     [esi], ecx
...
//loc_92E8B7:
//			  }
  __declspec(naked) comolokko ()
 {
//
//	_asm
//	{
// sub esp,08
//cmp dword ptr [0x00D74D40],00
//je loc_917A30
//push ebx
//push ebp
//push esi
...
//mov esi,[0x00D74D20] 
//add esp,10
//cmp dword ptr [esi+0x00040100],00
//je loc_917957
//jmp loc_917870
...
//mov eax,[0x00D74D20] 
//mov ebx,[ecx+ebx*4]
//mov ecx,[eax+0x00040100]
//add eax,0x000400F0 
//xor esi,esi
//cmp ecx,esi
//je loc_9178F2
//mov edi,[eax+08]
//add dword ptr [eax+0Ch],01
//mov edx,[eax+0Ch]
//add edi,edi
//add edi,edi
//cmp edi,edx
...
push 0xD74DEC
 push 01
 mov [esp+0x20],00000000
 mov eax,0x47D4A0
call eax

 mov esi,[0x0D74D20]
 add esp,0x10
 cmp dword ptr [esi+0x0040100],00
 je L00917957
 jmp L00917870
 lea esp,[esp+00000000]
 mov edi,edi
...
mov eax,[0x0D74D20]
mov ebx,[ecx+ebx*4]
mov ecx,[eax+0x0040100]
add eax,0x00400F0
xor esi,esi
cmp ecx,esi
je L009178F2
mov edi,[eax+0x08]
add dword ptr [eax+0x0C],01
mov edx,[eax+0x0C]
add edi,edi
add edi,edi
 cmp edi,edx
...
loc_92E671:                             // CODE XREF: sub_92E630+17Fj
				lea     ecx, [eax+400C4h]
                mov     [esp+18h], ebx
                mov     edx, ecx
                lea     esi, [eax+400C0h]
...
loc_92E71D:                             // CODE XREF: sub_92E630+A2j
                mov     eax, [esi+10h]
                lea     ecx, [esp+2Ch]
                push    eax
                mov     edx, [eax]
...
loc_92E74B:                             //; CODE XREF: sub_92E630+83j
									   // ; sub_92E630+EBj
//				mov     edx, [esp+10h]
//				lea     ecx, [esp+18h]
//				push    ecx
...
loc_92E76F:                             //; CODE XREF: sub_92E630+137j
                mov     eax, [edi+8]
                mov     cl, [eax]
                mov     [esp+14h], cl

loc_92E778:                             //; CODE XREF: sub_92E630+13Dj
//				mov     edx, [esp+14h]
//                and     edx, 0FFh
//				push    edx
//				push    offset aInvalidPacket_
...
loc_92E790:                             //; CODE XREF: sub_92E630+131j
                cmp     edi, ebx
                jz      short loc_92E79C
                mov     eax, [edi]
                push    1
...
loc_92E79C:                             //; CODE XREF: sub_92E630+162j
				cmp     ds:0xC491D5, bl
                jnz     short loc_92E7B5
                mov     eax, ds:0xC49128
                cmp     [eax+400ECh], ebx
...
loc_92E7B5:                             //; CODE XREF: sub_92E630+3Bj
										//; sub_92E630+172j
                mov     eax, ds:0xC4912C
                cmp     [eax+400ECh], ebx
                jz      loc_92E8B7
...
loc_92E83C:
					push 6
				call Cagirmesaj                       //; CODE XREF: sub_92E630+205j
                mov     eax, [esi+10h]
                lea     ecx, [eax+4]
                mov     [esi+10h], ecx
                mov     edx, [eax]
...
loc_92E8A6:                             //; CODE XREF: sub_92E630+20Aj
										//; sub_92E630+259j
				mov     eax, ds:0xC4912C
                cmp     [eax+400ECh], ebx
                jnz     loc_92E7C6

loc_92E8B7:                             //; CODE XREF: sub_92E630+10j
										//; sub_92E630+190j ...
                pop     edi
                pop     esi
                pop     ebp
...
bool bSendMessageToGame = true;
	...
	if(bSendMessageToGame)return CallWindowProcA((FARPROC)frmBot->dwGameWndProc,hwnd,uMsg,wParam,lParam);
...
{
	return CMainSocket.Client_Connect(strDNS,iServerPort);
}
...
void __fastcall TfrmBot::Button64Click(TObject *Sender)
{
BYTE Komut[]= {0x5B, 0x2, 0x1, 0x14, 0x27,
...
CInvSlot Item;
Item.SelectSlot(Edit53->Text.ToIntDef(0));

*(DWORD*)&Komut[5]= Item.dwItemNum();
Komut[9]=Edit53->Text.ToIntDef(0);
Item.SelectSlot(Edit54->Text.ToIntDef(0));

*(DWORD*)&Komut[10]=Item.dwItemNum();
Komut[14]=Edit54->Text.ToIntDef(0);
CMainSocket.Client_Send(Komut,55);
}
//---------------------------------------------------------------------------
