...
LPVOID Buffer = pBuf;
int Size = szBuf;
DWORD CallAddr = *(DWORD*)((GetIndex() * 4) + *(DWORD*)KO_SOCK + 0x4008C);
__asm{
MOV ECX,[KO_SOCK]
PUSH Size
PUSH Buffer
...
DWORD CArea::GetNearstNPCByID(int iIDArray[],int iIDCount, bool bAlive, FLOAT fMaxDist)
{
 ...
 #define _DWORD DWORD
int WINAPI sub_4AD0E0(int this1)
{
  int v1; // edx@1
  int result; // eax@2

  v1 = *(_DWORD *)(*(_DWORD *)this1 + 8);
  if ( v1 == *(DWORD*)(0xC3E7D4) )
  {
	for ( result = *(_DWORD *)(*(_DWORD *)this1 + 4);
		  *(_DWORD *)this1 == *(_DWORD *)(result + 8);
          result = *(_DWORD *)(result + 4) )
	  *(_DWORD *)this1 = result;
	if ( *(_DWORD *)(*(_DWORD *)this1 + 8) != result )
	  *(_DWORD *)this1 = result;
  }
  else
  {
	for ( result = *(_DWORD *)v1; result != *(DWORD*)(0xC3E7D4); result = *(_DWORD *)result )
      v1 = result;
	*(_DWORD *)this1 = v1;
  }
  return result;
...
void CRecvFunction::ChestDrop(char* pBuf){
	...
	if(frmBot->bWaitingNewDrop){
		if(GetTickCount() - frmBot->iLastChestTick < 2500)	//skip
			return;
		else{
	...
	if(frmBot->bMoveToChest){
		...
		if (!IsChestAdded(dwChestID))lstChestList.push_back(pNewChest);
	}
...
bool CTest::CheckSkillAvailable(DWORD dwSkillNum){
	if(!dwSkillNum)
		...
		// melih
		// hs yakalio amq bi de böyle deniyek xd
...
//	CODEREPLACE_END
	...
	// melih
	// hs snn ben aq :) ugrastýrdýgýn islere bak pic
