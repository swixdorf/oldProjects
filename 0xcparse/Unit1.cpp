...
String Buf = ListBox1->Items->Strings[i-1];
 ...
 String Buf2= ListBox1->Items->Strings[i-1].SubString(1,ListBox1->Items->Strings[i-1].AnsiPos("=")-1);
	...
	ListBox5->Items->Add(Buf2);
	}else{
	ListBox6->Items->Add(Buf2);
	}
...
if (ListBox5->Items->IndexOf(ListBox6->Items->Strings[i])==-1) {
 ListBox7->Items->Add(ListBox6->Items->Strings[i]);
}

}
ListBox7->Items->SaveToFile("end.txt");
}
//---------------------------------------------------------------------------
