...
function disassemble(var offset: dword; var description: string; var codelen: dword): string; overload;
...
begin
  ...
  startoffset:=offset;
  CopyMemory(addr(memory),pointer(offset),24);
//  readprocessmemory(diprocesshandle,pointer(offset),addr(memory),24,actualread);
   actualread:=24;
...
var len:dword ;
begin
  result:=disassemble(offset,ignore,len);
end;
